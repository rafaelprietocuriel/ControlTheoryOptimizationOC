...
resconnec=[];
resinit=[];
resper=[depvara(OCMATLC.statecoord,1)-depvarb(OCMATLC.statecoord,end); ...
    depvara(OCMATLC.costatecoord,1)-depvarb(OCMATLC.costatecoord,end)-freepar(end)*OCMATLC.initialcostatedifference; ...
    OCMATLC.fixvalue-depvara(OCMATLC.fixcoordinate,1)];
%-freepar(end)*OCMATLC.initialstatedifference
...
try
    if contnum==1
        save([OCMATLC.basicglobalvarfilename '4limitcycleuserI'],'MODELINFO')
    end
    save([OCMATLC.basicresultfilename '4limitcycleuserI'],'sout','bvpout')
catch
    failed=1;
end
