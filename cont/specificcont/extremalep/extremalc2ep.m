function out=extremalc2ep()
% continaue along a curve in the state space
out{1}=@operatoreq;
out{2}=@frechetder;
out{3}=@options;
out{4}{1}=@ode;
...
resinit=[depvara(OCMATAE.targetcoordinate)-freepar(end); ...
    OCMATAE.bcinitialpointcurve(depvara,modelpar,OCMATCONT.HE.arcarg(1))];
    ...
    switch ii
        case 1
            if ~isempty(OCMATAE.targetvalue)
                out=OCMATAE.targetvalue-freepar(end);
            end
        otherwise
            failed=1;
    end
...
if isempty(OCMATAE.movinghorizon)
    ...
    out.arcinterval=[OCMATAE.initialtime freepar(OCMATAE.switchtimecoord).' freepar(OCMATAE.switchtimecoord(end))+OCMATAE.movinghorizon];
end
%out.arcinterval=[OCMATAE.initialtime freepar(OCMATAE.switchtimecoord).' OCMATAE.truncationtime];
...
out.solverinfo.conttype='extremalc2ep';
...
try
    if contnum==1
        save([OCMATAE.basicglobalvarfilename '4extremalc2ep'],'MODELINFO')
    end
    save([OCMATAE.basicresultfilename '4extremalc2ep'],'sout','bvpout')
catch
    failed=1;
end
