...
arctime=[OCMATFTE.initialtime freepar(OCMATFTE.switchtimecoord).'];
...
if OCMATCONT.HE.numarc>1
    ...
    dxdt=OCMATFTE.canonicalsystem(t,depvar,modelpar,arcarg);
    Jpar(:,OCMATFTE.switchtimecoord)=dxdt;
end

%-------------------------------------------------------------------------
function res=bc(depvara,depvarb,freepar,modelpar)
global OCMATFTE OCMATCONT OCBVP
switchtimes=freepar(OCMATFTE.switchtimecoord(1:end-1));
timehorizon=freepar(OCMATFTE.switchtimecoord(end));
resconnec=[];

resinit=OCMATFTE.bcinitial(depvara,OCMATFTE.initialcoordinate,OCMATFTE.initialstate,modelpar,OCMATCONT.HE.arcarg(1));
restrans=OCMATFTE.bcinftransversality(timehorizon,depvarb,modelpar,OCMATCONT.HE.arcarg(end));
...
try
    if contnum==1
        save([OCMATFTE.basicglobalvarfilename '4extremalt4inft'],'MODELINFO')
    end
    save([OCMATFTE.basicresultfilename '4extremalt4inft'],'sout','bvpout')
catch
    failed=1;
end
