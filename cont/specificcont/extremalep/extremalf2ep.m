function out=extremalf2ep()
% extremalf2ep reach (some) states of the equilibrium in finite time
out{1}=@operatoreq;
out{2}=@frechetder;
out{3}=@options;
out{4}{1}=@ode;
...
if ~isempty(OCMATAE.endtimecoord)
    arctime=[OCMATAE.initialtime freepar(OCMATAE.switchtimecoord).' freepar(OCMATAE.endtimecoord)];
else
...
reshit=depvarb(OCMATAE.finitetimedepvarcoord,OCMATAE.hitarc)-OCMATAE.saddlepoint(OCMATAE.finitetimedepvarcoord);
if ~isempty(OCMATAE.asymptoticmatrix)
...
res=[resinit;resconnec;resasym;reshit];
