function sol=initocmat_AE_AE_PlusO(ocObj,ocAsym,contcoordinate,targetvalue,opt,varargin)
%
...
[sol approxobjective]=generatesolstruct(ocAsym);
if approxobjective
    o=objectivevalue(ocObj,sol,1);
    sol.y(end+1,:)=[0 cumsum((o(1:end-1)+o(2:end))/2.*diff(time(ocObj,sol,1)))];
end
OCMATAE.objectivevaluecoord=size(sol.y,1);
OCMATAE.objectivevaluecalc=true;
...
function [sol approxobjective]=generatesolstruct(ocTrj,varargin)
approxobjective=true;
...
if isfield(ocTrj.solverinfo,'objectivevaluecoord')
    approxobjective=false;
end
