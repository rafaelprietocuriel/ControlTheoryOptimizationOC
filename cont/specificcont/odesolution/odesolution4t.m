...
arctime=[OCMATODEBVP.initialtime freepar(OCMATODEBVP.switchtimecoord).'];
...
if OCMATCONT.HE.numarc>1
    ...
    Jpar(:,OCMATODEBVP.switchtimecoord)=dxdt+diffarctime(arc)*(s-arc)*Jt;
end

%-------------------------------------------------------------------------
function res=bc(depvara,depvarb,freepar,modelpar)
global OCMATODEBVP OCMATCONT OCBVP
switchtimes=freepar(OCMATODEBVP.switchtimecoord(1:end-1));
resconnec=[];

resinit=depvara(OCMATODEBVP.fixinitstatecoord,1)-OCMATODEBVP.initialstate;
resend=depvarb(OCMATODEBVP.fixendstatecoord,end)-OCMATODEBVP.endstate;
...
res=[resinit;resconnec;resend(:);resper];
%-------------------------------------------------------------------------
...
for ii=id
    switch ii
        case 1
            out=freepar(OCMATCONT.HE.numparameter)-OCMATODEBVP.targetvalue;
        otherwise
            out=[];
            failed=1;
    end
...
out.modelparameter=OCMATODEBVP.parametervalue;
...
out.solverinfo.conttype='odesolution4t';
...
try
    if contnum==1
        save([OCMATODEBVP.basicglobalvarfilename 'odesolution4t'],'MODELINFO')
    end
    save([OCMATODEBVP.basicresultfilename 'odesolution4t'],'sout','bvpout')
catch
    failed=1;
end
