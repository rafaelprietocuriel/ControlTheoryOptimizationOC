...
if OCMATODEBVP.changeparameterindex
...
switch OCMATODEBVP.conttype
    case 0
        initialstate=OCMATODEBVP.startvalue+freepar(end)*OCMATODEBVP.continuationvector;
        resinit=depvara(OCMATODEBVP.contcoordinate,1)-initialstate;
    case 1
        endstate=OCMATODEBVP.startvalue+freepar(end)*OCMATODEBVP.continuationvector;
        resend=depvarb(OCMATODEBVP.contcoordinate,end)-endstate;
end
resinit=[resinit; ...
    depvara(OCMATODEBVP.fixinitstatecoord,1)-OCMATODEBVP.fixinitialstate];
resend=[resend; ...
    depvarb(OCMATODEBVP.fixendstatecoord,end)-OCMATODEBVP.fixendstate];
...
[t,y,z,freepar,modelpar]=drearr(tmesh,coeff);
if ~isempty(OCMATODEBVP.usertarget)
    out=OCMATODEBVP.usertarget(t,y,modelpar,OCMATCONT.HE.arcarg,freepar);
    ...
    switch ii
        case 1
            if ~isempty(OCMATODEBVP.usertarget)
                ...
                out=freepar(OCMATCONT.HE.numparameter)-1;
            end
        otherwise
            out=[];
            failed=1;
...
out.solverinfo.conttype='odesolution4s';
...
out.solverinfo.periodicstatecoord=OCMATODEBVP.periodicstatecoord;
out.solverinfo.periodicstatecoord=OCMATODEBVP.changeparameterindex;
...
try
    if contnum==1
        save([OCMATODEBVP.basicglobalvarfilename 'odesolution4s'],'MODELINFO')
    end
    save([OCMATODEBVP.basicresultfilename 'odesolution4s'],'sout','bvpout')
catch
    failed=1;
end
