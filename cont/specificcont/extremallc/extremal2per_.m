...
for arc=1:OCMATCONT.HE.numarc
    ...
    % test arctimes
    violationmat=diffarctime<-OCMATCONT.OPTIONS.zerotimedifftolerance;
    if any(violationmat)
        counter=counter+1;
        ...
        infoS(counter).cols=cols;
        infoS(counter).violationmat=violationmat;
        infoS(counter).constraintvalue=diffarctime;
...
try
    if contnum==1
        save([OCMATAE.basicglobalvarfilename '4extremal2per'],'MODELINFO')
    end
    save([OCMATAE.basicresultfilename '4extremal2per'],'sout','bvpout')
catch
    failed=1;
end
