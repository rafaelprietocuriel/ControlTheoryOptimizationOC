function sol=initocmat_AE_AE_PER(ocObj,ocAsym,contcoordinate,targetvalue,opt,varargin)
%
...
ocPerarcarg=arcargument(ocAsym);
...
timesettransformation.asymptoticapproximation=ocAsym.arcinterval(end);
% initialize solution using the repeated entries of the equilibrium
ocPer=limitset(ocAsym);
...
if ~isempty(freeendtime)
    ...
    depvarper=dependentvar(ocPer);
    OCMATAE.distance=norm(depvarper(:,end)-depvar(:,end));
...
if isempty(asymptoticmatrix)
    ...
    asymptoticmatrix=asymptoticbc(J,pathtpe,'d',ZeroDeviationTolerance);
end
if ~isempty(fixedcoord)
...
if objectivevaluecalc && length(sol.y(:,1))==2*statenum(ocObj)
    o=objectivevalue(ocObj,ocAsym,1);
    sol.y(end+1,:)=[0 cumsum((o(1:end-1)+o(2:end))/2.*diff(time(ocObj,ocAsym,1)))];
end
if ~objectivevaluecalc && length(sol.y(:,1))>numode
