function sol=initocmat_EP_FT_HET(ocObj,ocMP,parindex,fixcoordinate,opt,varargin)
%
...
for ii=1:hetorder
    if isocasymptotic(ocMP(ii))
        ...
        if isempty(pathtpe{ii})
            answer=input('Stability of path (s)/u: ','s');
            while 1
                if isempty(answer)
                    % default value 'y'
                    answer='s';
        ...
        switch pathtpe{ii}
            case 's'
                OCMATHET.stableflag{ii}=1;
            case {'u','stu'}
                OCMATHET.stableflag{ii}=0;
        end
        OCMATHET.ocasymptotic(ii)=1;
    else
        OCMATHET.stableflag{ii}=[];
        OCMATHET.ocasymptotic(ii)=0;
    end
...
if ~isempty(optimalhorizonidx)
    ...
    optimalhorizon=zeros(1,hetorder);
end
...
OCMATHET.bcoptimalhorizon=funch{5}{6};
...
OCMATHET.hitstatevalue=hitstatevalue;
OCMATHET.hitstatecoordinate=hitstatecoordinate;

OCMATHET.statecoord=statecoord(ocObj);
OCMATHET.costatecoord=costatecoord(ocObj);

sol=generatesolstruct(ocMP);
% mode and path specific variables
OCMATCONT.HE.edge=cell(1,hetorder);
OCMATHET.optimalhorizon=optimalhorizon;
...
for ii=1:hetorder
    ...
    if isocasymptotic(ocMP(ii))
        ...
        OCMATHET.fixendstatecoord{ii}=[];
        OCMATHET.endstate{ii}=[];
    else
        depvar=dependentvar(ocMP(ii));
        OCMATHET.fixendstatecoord{ii}=fixendstate{ii};
        if ~isempty(fixendstate)
            OCMATHET.endstate{ii}=depvar(fixendstate{ii},end);
        end
        OCMATHET.asymptoticmatrix{ii}=[];
        OCMATHET.orthspacedim{ii}=[];
        OCMATHET.Y{ii}=[];
        OCMATHET.Id{ii}=[];
        OCMATHET.numY{ii}=[];
        OCMATHET.inftimetransformation(ii)=0;
...
for ii=1:hetorder
    if optimalhorizon(ii)
        ...
        OCMATHET.optimalhorizoncoord{ii}=length(parameters);
    else
        OCMATHET.truncationtime(ii)=truncationtime(ii);
        OCMATHET.optimalhorizoncoord{ii}=[];
    end
    ...
    if ~isempty(switchtimes{ii})
        OCMATHET.switchtimecoord{ii}=length(parameters)+(1:length(switchtimes{ii}));
...
OCMATHET.initialcostatedifference=ocMP(2).y(OCMATHET.costatecoord,1)-ocMP(1).y(OCMATHET.costatecoord,1);
...
for ii=2:nummult
    ...
    sol.y=[sol.y tmpy([OCMATHET.statecoord;OCMATHET.costatecoord],:)];
