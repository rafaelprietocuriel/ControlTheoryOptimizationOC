...
for ii=1:OCMATHET.hetorder
    ...
    if mod(ii,OCMATHET.connectionnum)
        ctr=ctr+1;
        resinit=[resinit; ...
            depvara(:,OCMATHET.initcoord(ii+1))-depvara(:,OCMATHET.initcoord(ii))];
        if ~isempty(OCMATHET.fixcoordinate)
            resinit=[resinit; ...
                OCMATHET.fixvalue(:,ctr)-depvara(OCMATHET.fixcoordinate,OCMATHET.initcoord(ii))];
        end
        hatx{ctr}=freepar(OCMATHET.equilibriumcoord{ctr});
        resequilibrium=[resequilibrium;OCMATHET.canonicalsystem(0,hatx{ctr},modelpar,arcarg(end))];
...
for ii=1:OCMATHET.hetorder
    ...
    if OCMATHET.simple
        ...
        Jac=OCMATHET.canonicalsystemjacobian(0,hatx{OCMATHET.equilibriumidx(ii)},modelpar,arcarg(end));
        resricatti=[resricatti;ricatti(Y,Jac,OCMATHET.Q0{ii},OCMATHET.subspacedim{ii})];
    end
    resasym=[resasym;OCMATHET.bcasymptotic(depvarb(:,OCMATHET.cumsumnumarc(ii)),asymptoticmatrix,hatx{OCMATHET.equilibriumidx(ii)})];
    ...
    if OCMATHET.movinghorizon(ii)
        resasym=[resasym; ...
            sqrt(sum((hatx{OCMATHET.equilibriumidx(ii)}-depvarb(:,OCMATHET.cumsumnumarc(ii))).^2))-OCMATHET.distance(ii)];
    end
end
if ~isempty(OCMATHET.userconstraint)
    resuser=OCMATHET.userconstraint(depvara,depvarb,modelpar);
end
res=[resinit;resasym;resconnec;resequilibrium;resricatti;resuser];
%-------------------------------------------------------------------------
...
for ii=1:OCMATHET.hetorder
    hatx=freepar(OCMATHET.equilibriumcoord{OCMATHET.equilibriumidx(ii)});
...
out.solverinfo.conttype='heterocliniccyc';
...
if ~OCMATHET.simple
    ...
    out.solverinfo.asymptoticmatrix=OCMATHET.asymptoticmatrix;
end
...
try
    if contnum==1
        save([OCMATHET.basicglobalvarfilename '4heterocliniccyc'],'MODELINFO')
    end
    save([OCMATHET.basicresultfilename '4heterocliniccyc'],'sout','bvpout')
catch
    failed=1;
end
