function sol=initocmat_EP_HET_CYC(ocObj,ocMP,parindex,fixcoordinate,varargin)
%
...
sol=[];
if isimplicit(ocObj)
    OCMATHET.implicit=true;
    sol=initocmat_AE_HET4implicit(ocObj,ocMP,parindex,fixcoordinate,varargin{:});
    return
else
    OCMATHET.implicit=false;
end
...
targetparameteridx=find(strcmpi(varargin,'targetparameter'));
...
hopfidx=find(strcmpi(varargin,'hopf'));
fixlccoordinateidx=find(strcmpi(varargin,'fixlccoordinate'));
...
if ~isempty(targetparameteridx)
    targetparameter=varargin{targetparameteridx+1};
else
    targetparameter=[];
end
...
if isempty(targetparameter)
    targetparameter=parindex(end);
end
if ~isempty(fixdistanceidx)
    fixdistance=varargin{fixdistanceidx+1};
end
if isempty(freeendtime)
    freeendtime=zeros(1,hetorder);
elseif numel(freeendtime)==1
    freeendtime=repmat(freeendtime,1,hetorder);
end
if ~isempty(hopfidx)
    hopf=varargin{hopfidx+1};
end
if ~isempty(fixlccoordinateidx)
    fixlccoordinate=varargin{fixlccoordinateidx+1};
end
...
for ii=1:length(OCMATHET.fixdistance)
    OCMATHET.saddlepoint{ii}=dependentvar(limSet{octrajectory2limset(ii,2)});
    OCMATHET.saddlepoint{ii}=OCMATHET.saddlepoint{ii}(:,1);
    if OCMATHET.fixdistance(ii)
        OCMATHET.distance(ii)=norm(OCMATHET.saddlepoint{ii}-ocMP(ii).octrajectory.y(:,end));
    end
end
...
for ii=1:hetorder
    ...
    [asymptoticmatrix,OCMATHET.numstable{ii},OCMATHET.numunstable{ii},OCMATHET.numcenter{ii},infoStruct]=asymptoticbc(linearization(limSet{octrajectory2limset(ii,2)}),pathtpe{ii},maptype,ZeroDeviationTolerance(ii),AsymptoticBCMethod{ii});
...
for ii=1:hetorder/2
    ...
    %OCMATHET.initialstatedifference(:,ii)=depvar2(:,1)-depvar1(:,1);
    OCMATHET.initialcostatedifference(:,ii)=depvar2(OCMATHET.costatecoordinate,1)-depvar1(OCMATHET.costatecoordinate,1);
    OCMATHET.fixvalue(ii)=depvar1(fixcoordinate,1);
end
