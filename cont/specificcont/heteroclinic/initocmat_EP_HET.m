function sol=initocmat_EP_HET(ocObj,ocMP,parindex,fixcoordinate,opt,varargin)
%
...
movinghorizon=[];
userbc=[]; %the user function
targetfunction=[]; %the user function
testhopf=[];
testeig=[];
simpleflag=[];
...
% user provided boundary condition(s)
OCMATHET.userbc=userbc;
OCMATHET.targetfunction=targetfunction;
...
OCMATHET.targetparameterindex=parameterindex(ocObj,targetparameterindex);
OCMATHET.simple=simpleflag;
OCMATHET.movinghorizon=movinghorizon;
OCMATHET.testhopf=testhopf;
OCMATHET.testeig=testeig;
