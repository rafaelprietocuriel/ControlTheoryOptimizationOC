...
%J=OCMATINDIF.JX;
J=dtds*OCMATINDIF.canonicalsystemjacobian(t,depvar,modelpar,arcarg);
...
Jpar=zeros(size(depvar,1),length(freepar));
...
for order=1:OCMATINDIF.indifferenceorder
    ...
    if OCMATINDIF.implicit
        resae=[resae; 
            OCMATINDIF.algebraicequation(depvara(:,OCMATINDIF.initcoord(order)),modelpar,arcarg(1))];
    end
...
%             depvara(OCMATINDIF.statecoordinate,OCMATINDIF.initcoord(order+1))-depvara(OCMATINDIF.statecoordinate,OCMATINDIF.initcoord(order))];
    ...
    if OCMATINDIF.exogenousfunction
        resinit=[resinit; ...
            depvara(OCMATINDIF.exogenousdynamicscoordinate,OCMATINDIF.arccoord{order}(1))-OCMATINDIF.exogenousinitialstatesfunc(depvara(:,OCMATINDIF.arccoord{order}),depvarb(:,OCMATINDIF.arccoord{order}),modelpar,arcarg)];
        %         resinit=[resinit; ...
    ...
    if ~isempty(OCMATINDIF.divergingcoordinate)
        if ~isempty(OCMATINDIF.divergingcoordinate{order})
            resasym=[resasym; ...
                OCMATINDIF.bcinf(depvarb(:,OCMATINDIF.cumsumnumarc(order)),OCMATINDIF.fixendcoordinate{order},OCMATINDIF.endvalue{order},modelpar,arcarg(end))];
        end
    end
    try
        ...
        resasym=[resasym;OCMATINDIF.bcasymptotic(depvarb(:,OCMATINDIF.arccoord{order}),OCMATINDIF.asymptoticmatrix{order},OCMATINDIF.saddlepoint{order},modelpar,arcarg(end))];
    end
    if OCMATINDIF.fixdistance(order)>0
        if isempty(OCMATINDIF.divergingcoordinate{order})
            resasym=[resasym; ...
                sqrt(sum((OCMATINDIF.saddlepoint{order}(OCMATINDIF.statecostatecoord)-depvarb(OCMATINDIF.statecostatecoord,OCMATINDIF.cumsumnumarc(order))).^2))-OCMATINDIF.distance{order}];
        else
            convergingcoordinate=setdiff(OCMATINDIF.statecostatecoord,OCMATINDIF.divergingcoordinate{order});
            yend=depvarb(:,OCMATINDIF.arccoord{order});
            hatx=OCMATINDIF.saddlepoint{order};
            %yend(OCMATINDIF.divergingcoordinate{order},:)=[];
            %hatx(OCMATINDIF.divergingcoordinate{order},:)=[];
            resasym=[resasym; ...
                sqrt(sum((yend(convergingcoordinate)-hatx(convergingcoordinate)).^2))-OCMATINDIF.distance{order}];
        end
    end
    for arc=1:OCMATINDIF.numarc(order)-1
        ...
        if OCMATINDIF.objectivevaluecalc
            resconnec=[resconnec; ...
                depvarb(OCMATINDIF.objectivevaluecoord,OCMATINDIF.arccoord{order}(arc))-depvara(OCMATINDIF.objectivevaluecoord,OCMATINDIF.arccoord{order}(arc+1))];
        end
        if OCMATINDIF.exogenousfunction
            resconnec=[resconnec; ...
                depvarb(OCMATINDIF.exogenousdynamicscoordinate,OCMATINDIF.arccoord{order}(arc))-depvara(OCMATINDIF.exogenousdynamicscoordinate,OCMATINDIF.arccoord{order}(arc+1))];
        end
    end
end
res=[restarget;resinit;resasym;resconnec;residpt;resae];
%-------------------------------------------------------------------------
...
for arc=1:OCMATCONT.HE.numarc
    ...
    if ~OCBVP.multiarccalc
        ...
        [constr,labelS]=OCMATINDIF.testadmissibility(t,sol.y(:,leftarcindex(arc):rightarcindex(arc)),modelpar,arcarg);
    end
