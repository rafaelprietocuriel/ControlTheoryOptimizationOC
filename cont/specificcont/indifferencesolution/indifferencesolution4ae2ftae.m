function out=indifferencesolution4ae2ftae()
...
restarget=[];
if ~isempty(freepar)
    restarget=OCMATINDIF.bcinitial(depvara,OCMATINDIF.targetcoordinate,OCMATINDIF.startvalue+freepar(end)*OCMATINDIF.continuationvector,modelpar,OCMATCONT.HE.arcarg([OCMATINDIF.arccoord{:}]),OCMATINDIF.initcoord([1 2]));
end
for ii=1:OCMATINDIF.indifferenceorder
    ...
    if OCMATINDIF.ftequilibriumindex(ii)
        resasym=[resasym;OCMATINDIF.bcasymptotic(depvarb(:,OCMATINDIF.cumsumnumarc(ii)),OCMATINDIF.asymptoticmatrix{ii},OCMATINDIF.saddlepoint{ii})];
    else
        resasym=[resasym;OCMATINDIF.bcasymptotic(depvarb(:,OCMATINDIF.cumsumnumarc(ii)),OCMATINDIF.asymptoticmatrix{ii},OCMATINDIF.saddlepoint{ii})];
    end
    if OCMATINDIF.ftequilibriumindex(ii)
...
out.solverinfo.conttype='indifferencesolution4ae2ftae';
...
try
    if contnum==1
        save([OCMATINDIF.basicglobalvarfilename '4indifferencesolution4ae2ftae'],'MODELINFO')
    end
    save([OCMATINDIF.basicresultfilename '4indifferencesolution4ae2ftae'],'sout','bvpout')
catch
    failed=1;
end
