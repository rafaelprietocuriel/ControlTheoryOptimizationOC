function out=indifferencesolution2lc()
...
if OCMATINDIF.numarc(solutionindex)>1
    ...
    if relarc<OCMATINDIF.numarc(solutionindex)
        ...
        elseif limitcycleindex
            dxdt=OCMATINDIF.canonicalsystem(t,depvar,modelpar,arcarg);
            Jpar(:,OCMATINDIF.periodcoordinate(limitcycleindex))=dxdt;
        end
    end
else
    if OCMATINDIF.freeendtime(solutionindex)
...
if OCMATINDIF.limitcyclecounter
    ...
    for ii=1:OCMATINDIF.limitcyclecounter
        ...
        switchtimes=freepar(OCMATINDIF.switchtimecoordinate{1+ii});
        arcarg=OCMATINDIF.arcarg{1+ii};
        initaldepvarlc(:,ii)=actdepvara(:,1);
        resper=[resper; ...
            OCMATINDIF.bclimitcycle(actdepvara,actdepvarb,modelpar,arcarg)];
        if ~OCMATINDIF.fixlccoordinate(ii)
            ...
            resper=[resper; ...
                actdepvara(OCMATINDIF.fixlccoordinate(ii),1)-OCMATINDIF.fixlcvalue(ii)];
        end

        for arc=1:OCMATINDIF.numarc(1+ii)-1
            resconnec=[resconnec; ...
                OCMATINDIF.reset(actdepvara,actdepvarb,modelpar,switchtimes,arcarg,OCMATINDIF.edge{1+ii},arc); ...
                OCMATINDIF.guard(actdepvara,actdepvarb,modelpar,switchtimes,arcarg,OCMATINDIF.edge{1+ii},arc)];
        end
...
for ii=1:OCMATINDIF.indifferenceorder
    if ~isempty(OCMATINDIF.pathtype{ii})
        ...
        if ctr==1 && ~isempty(OCMATINDIF.fixinitstate)
            resinit=[resinit;...
        ...
        if ~isempty(OCMATINDIF.freeparameter)
            ...
            if OCMATINDIF.equilibriumcounter>=1
    ...
    if ii>1
        ...
        if OCMATINDIF.contindifferencevalue
            indifferencevalue=OCMATINDIF.indifferencevalue+freepar(end)*OCMATINDIF.indifferencevaluevector;
        else
            indifferencevalue=OCMATINDIF.indifferencevalue;
        end
        residpt=[residpt; ...
            OCMATINDIF.bcindifference(initialdepvar,modelpar,initialarcarg,[ii ii-1])-indifferencevalue];
    end
end
...
for ii=1:OCMATINDIF.indifferenceorder
    if ~isempty(OCMATINDIF.pathtype{ii})
        ...
        if ~isempty(checkcoordinate)
            if~isempty(OCMATINDIF.freeparameter)
                hatx=freepar(OCMATINDIF.equilibriumcoordinate{ctr});
            else
                hatx=OCMATINDIF.saddlepoint{ii};
            end
...
out.solverinfo.conttype='indifferencesolution2lc';
...
try
    if contnum==1
        save([OCMATINDIF.basicglobalvarfilename '4indifferencesolution2lc'],'MODELINFO')
    end
    save([OCMATINDIF.basicresultfilename '4indifferencesolution2lc'],'sout','bvpout')
catch
    failed=1;
end
