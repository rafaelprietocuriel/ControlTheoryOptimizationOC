function out=indifferencesolution4per()
...
if OCMATINDIF.numarc(solutionindex)>1
    ...
    if relarc<OCMATINDIF.numarc(solutionindex)
        Jpar(OCMATCONT.DOMAINDDATA(arcindex).eqcoord,OCMATINDIF.switchtimecoord{solutionindex}(relarc))=dxdt+diffarctime(relarc)*(s-transformedtimeshift-relarc+1)*Jt;
        if relarc>1
            Jpar(OCMATCONT.DOMAINDDATA(arcindex).eqcoord,OCMATINDIF.switchtimecoord{solutionindex}(relarc-1))=-(dxdt+diffarctime(relarc)*(s-transformedtimeshift-relarc)*Jt);
        end
    else
...
restarget=OCMATINDIF.bcinitial(depvara,OCMATINDIF.targetcoordinate,OCMATINDIF.startvalue+freepar(end)*OCMATINDIF.continuationvector);
for order=1:OCMATINDIF.indifferenceorder
    ...
    if ~isempty(OCMATINDIF.switchtimecoord{order})
    ...
    %resinit=[resinit;depvara(end,OCMATINDIF.initcoord(order))];
    if order<OCMATINDIF.indifferenceorder
        ...
        residpt=[residpt; ...
            OCMATINDIF.bcindifference(depvarb,modelpar,OCMATCONT.HE.arcarg([OCMATINDIF.arccoord{order:order+1}]),OCMATINDIF.endcoord([order order+1]))+OCMATINDIF.objectivevalueper(order)-OCMATINDIF.objectivevalueper(order+1)];
    end
...
colcounter=colcounter+domainddata(1).numeq*OCMATCONT.HE.numarc;
for arc=1:OCMATCONT.HE.numarc
    solutionindex=OCMATINDIF.solutionindex(arc);
    arcindex=OCMATCONT.HE.arcindex(arc);
    arcarg=OCMATCONT.HE.arcarg(arc);
    if arc==1
        [Ja1part Jb1part Jpar1tmp]=OCMATINDIF.bcjacobianinitial(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,OCMATINDIF.targetcoordinate,OCMATINDIF.continuationvector);
        [Ja2part Jb2part Jpar2tmp]=OCMATINDIF.bcjacobianasymptotic(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge{solutionindex},OCMATINDIF.saddlepoint(:,solutionindex),OCMATINDIF.asymptoticmatrix{solutionindex}.',arc);
        Jpar(1:OCMATCONT.HE.numboundarycondition(1),:)=[Jpar1tmp;Jpar2tmp];
        rowcounterstart=rowcounter+1;
        rowcounter=rowcounter+OCMATCONT.HE.numboundarycondition(1);
        Ja(rowcounterstart:rowcounter,colidx_start:colcounter)=[Ja1part;Ja2part];
        Jb(rowcounterstart:rowcounter,colidx_start:colcounter)=[Jb1part;Jb2part];
    end
    if ~isempty(OCMATCONT.HE.edge{solutionindex})
        if arc==1
            Jresetpart=OCMATINDIF.jacobianreset(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,0);
            Jguardpart=OCMATINDIF.jacobianguard(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,0);
            rowcounterstart=rowcounter+1;
            rowcounter=rowcounter+domainddata(arcindex).numode;
            Jb(rowcounterstart:rowcounter,colidx_start:colcounter)=Jresetpart;
            rowcounterstart=rowcounter+1;
            rowcounter=rowcounter+1;
            Jb(rowcounterstart:rowcounter,colidx_start:colcounter)=Jguardpart;
        elseif arc==OCMATCONT.HE.numarc
            rowcounter=rowcounter-domainddata(arcindex).numode-1;
            Jresetpart=OCMATINDIF.jacobianreset(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,-1);
            Jguardpart=OCMATINDIF.jacobianguard(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,-1);
            rowcounterstart=rowcounter+1;
            rowcounter=rowcounter+domainddata(arcindex).numode;
            Ja(rowcounterstart:rowcounter,colidx_start:colcounter)=Jresetpart;
            rowcounterstart=rowcounter+1;
            rowcounter=rowcounter+1;
            Ja(rowcounterstart:rowcounter,colidx_start:colcounter)=Jguardpart;
        else
            rowcounter=rowcounter-domainddata(arcindex).numode-1;
            Jaresetpart=OCMATINDIF.jacobianreset(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,-1);
            Jbresetpart=OCMATINDIF.jacobianreset(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,0);
            Jaguardpart=OCMATINDIF.jacobianguard(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,-1);
            Jbguardpart=OCMATINDIF.jacobianguard(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge,arc,0);
            rowcounterstart=rowcounter+1;
            rowcounter=rowcounter+domainddata(arcindex).numode;
    ...
    if arc==OCMATCONT.HE.numarc
        [Ja1part Jb1part]=OCMATINDIF.bcjacobianindifference(depvara,depvarb,modelpar,freepar,OCMATCONT.HE.arcarg([arc-1 arc]),OCMATCONT.HE.edge,OCMATINDIF.cumsumnumarc);
        [Ja2part Jb2part]=OCMATINDIF.bcjacobianasymptotic(depvara,depvarb,modelpar,freepar,arcarg,OCMATCONT.HE.edge{solutionindex},OCMATINDIF.saddlepoint(:,solutionindex),OCMATINDIF.asymptoticmatrix{solutionindex}.',arc);
        rowcounterstart=rowcounter+1;
        rowcounter=rowcounter+OCMATCONT.HE.numboundarycondition(end);
        Ja(rowcounterstart:rowcounter,colidx_start:colcounter)=[Ja1part;Ja2part];
        Jb(rowcounterstart:rowcounter,colidx_start:colcounter)=[Jb1part;Jb2part];
    end
end
...
try
    if contnum==1
        save([OCMATINDIF.basicglobalvarfilename '4indifferencesolution4per'],'MODELINFO')
    end
    save([OCMATINDIF.basicresultfilename '4indifferencesolution4per'],'sout','bvpout')
catch
    failed=1;
end
