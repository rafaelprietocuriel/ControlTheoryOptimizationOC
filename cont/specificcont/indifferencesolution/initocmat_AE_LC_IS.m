function sol=initocmat_AE_LC_IS(ocObj,ocMP,opt,varargin)
%
...
for ii=1:indifforder
    ...
    OCMATINDIF.objectivevalueper(ii)=exp(-r*arct)*ov/(1-exp(-r*tau));
...
for ii=1:indifforder
    if ~isempty(freeendtime)
        OCMATINDIF.truncationtimecoord{ii}=paroffset+ii;
    end
end
sol.parameters=[sol.parameters sol.arcinterval(righttimeindex)];
