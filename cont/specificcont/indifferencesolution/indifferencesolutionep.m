function out=indifferencesolutionep()
...
if ~OCMATINDIF.freeendtime
    arctime=[OCMATINDIF.initialtime freepar(OCMATINDIF.switchtimecoord).' OCMATINDIF.truncationtime];
else
    arctime=[OCMATINDIF.initialtime freepar(OCMATINDIF.switchtimecoord).' freepar(OCMATINDIF.freeendtimecoord)];
end
...
if OCMATCONT.HE.numarc>1
    ...
    if arc<OCMATCONT.HE.numarc
        Jpar(OCMATCONT.DOMAINDDATA(1).eqcoord,OCMATINDIF.switchtimecoord(arc))=dxdt+diffarctime(arc)*(s-arc)*Jt;
        if arc>1
            Jpar(OCMATCONT.DOMAINDDATA(1).eqcoord,OCMATINDIF.switchtimecoord(arc-1))=-(dxdt+diffarctime(arc)*(s-arc-1)*Jt);
        end
    else
        Jpar(OCMATCONT.DOMAINDDATA(1).eqcoord,OCMATINDIF.switchtimecoord(arc-1))=-(dxdt+diffarctime(arc)*(s-arc-1)*Jt);
        if OCMATINDIF.freeendtime
            dxdt=OCMATINDIF.canonicalsystem(t,depvar,modelpar,arcarg);
            Jpar(:,OCMATINDIF.freeendtimecoord)=dxdt;
        end
    end
else
    if OCMATINDIF.freeendtime
...
global OCMATINDIF OCMATCONT OCBVP
modelpar(OCMATINDIF.freeparameterindex)=freepar(OCMATINDIF.freeparametercoordinate);
switchtimes=freepar(OCMATINDIF.switchtimecoord);

resconnec=[];
hatx=freepar(OCMATCONT.HE.equilibriumcoord);
Y=freepar(OCMATCONT.HE.Ycoord);
asymptoticmatrix=OCMATINDIF.Q0*[-Y';OCMATINDIF.Id];
Jac=OCMATINDIF.canonicalsystemjacobian(0,hatx,modelpar,OCMATCONT.HE.arcarg(end));

resinit=OCMATINDIF.indifferenceequilibrium(depvara(:,1),modelpar,OCMATCONT.HE.arcarg(1)); % conditions that the state path starts at the stalling equilibrium and that the inner control lies on the boundary

resricatti=ricatti(Y,Jac);
resequilibrium=OCMATINDIF.equilibrium(hatx,modelpar,OCMATCONT.HE.arcarg(end));
%resinit=OCMATINDIF.bcinitial(depvara,OCMATINDIF.initialcoordinate,hatx(OCMATINDIF.initialcoordinate),modelpar,OCMATCONT.HE.arcarg(1));
if OCMATINDIF.objectivevaluecalc
    resinit=[resinit;depvara(end,1)];
end

resasym=OCMATINDIF.bcasymptotic(depvarb,asymptoticmatrix,hatx);

if OCMATINDIF.freeendtime
    yend=depvarb(:,end);
    resasym=[resasym; ...
        sqrt(sum((yend-hatx).^2))-OCMATINDIF.distance];
end

for ii=1:numel(OCMATCONT.HE.arcarg)-1
    resconnec=[resconnec;
        OCMATINDIF.reset(depvara,depvarb,modelpar,switchtimes,OCMATCONT.HE.arcarg,OCMATCONT.HE.edge,ii); ...
        OCMATINDIF.guard(depvara,depvarb,modelpar,switchtimes,OCMATCONT.HE.arcarg,OCMATCONT.HE.edge,ii)];
end
...
for arc=1:OCMATCONT.HE.numarc
    ...
    % test arctimes
    if strcmp(OCMATINDIF.pathtype,'u')
...
h=OCMATINDIF.plotcontinuation(t,sol.y,modelpar,OCMATCONT.HE.arcarg,freepar,tangent);
...
if ~isempty(OCMATINDIF.targetparametervalue)
    out=OCMATINDIF.targetparametervalue-modelpar(OCMATINDIF.targetparameterindex);
...
if ~OCMATINDIF.freeendtime
    out.arcinterval=[OCMATINDIF.initialtime freepar(OCMATINDIF.switchtimecoord).' OCMATINDIF.truncationtime];
else
    out.arcinterval=[OCMATINDIF.initialtime freepar(OCMATINDIF.switchtimecoord).' freepar(OCMATINDIF.freeendtimecoord)];
    out.solverinfo.distance=OCMATINDIF.distance;
    out.solverinfo.freeendtimecoord=OCMATINDIF.freeendtimecoord;
end
...
out.solverinfo.conttype='indifferencesolutionep';
...
OCMATCONT.adapted = 1;
% 
[U,S,V]=svd(OCMATINDIF.Q0(:,1:OCMATINDIF.subspacedim)+OCMATINDIF.Q0(:,OCMATINDIF.subspacedim+1:end)*Y);
OCMATINDIF.Q0=U;
OCMATINDIF.Y=zeros(OCMATINDIF.orthspacedim,OCMATINDIF.subspacedim);

freepar(OCMATCONT.HE.Ycoord)=OCMATINDIF.Y;
...
try
    if contnum==1
        save([OCMATINDIF.basicglobalvarfilename '4indifferencesolutionep'],'MODELINFO')
    end
    save([OCMATINDIF.basicresultfilename '4indifferencesolutionep'],'sout','bvpout')
catch
    failed=1;
end
...
if  ~isempty(Y)
    ...
    [R11,R12,R21,R22]=RicattiCoeff(OCMATINDIF.Q0,J,OCMATINDIF.subspacedim);
...
[dHdx,dHdp]=OCMATINDIF.hamiltonianderivative(t(1),y(:,1),modelpar,arcarg);
out=dHdx(:).'*tangent1(1:end-1)+dHdp(OCMATINDIF.freeparameterindex)*tangent1(end);
