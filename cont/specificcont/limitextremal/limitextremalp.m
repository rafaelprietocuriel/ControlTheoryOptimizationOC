...
modelpar(OCMATLSC.varyparameterindex)=freepar(OCMATLSC.parametercoord);
...
Jpar(:,OCMATCONT.HE.numparameter)=Jmodelpar(:,OCMATLSC.varyparameterindex);
...
Hypar(:,:,OCMATLSC.parametercoord)=DJpar(:,:,OCMATLSC.varyparameterindex);
...
asymptoticmatrix=OCMATLSC.Q0*[-Y';OCMATLSC.Id];
Jac=OCMATLSC.canonicalsystemjacobian(0,hatx,modelpar,OCMATCONT.HE.arcarg(end));
resequilibrium=OCMATLSC.equilibrium(hatx,modelpar,OCMATCONT.HE.arcarg(end));
...
try
    resasym=OCMATLSC.bcasymptotic(depvarb,asymptoticmatrix,hatx);
catch
    resasym=OCMATLSC.bcasymptotic(depvarb,asymptoticmatrix,hatx,modelpar,OCMATCONT.HE.arcarg(end),depvara);
end
...
out.solverinfo.subspacedim=OCMATLSC.subspacedim;
out.solverinfo.orthspacedim=OCMATLSC.orthspacedim;
out.solverinfo.qbasis=OCMATLSC.Q0;
...
try
    if contnum==1
        save([OCMATLSC.basicglobalvarfilename '4limitextremalp'],'MODELINFO')
    end
    save([OCMATLSC.basicresultfilename '4limitextremalp'],'sout','bvpout')
catch
    failed=1;
end
...
if  ~isempty(Y)
    ...
    [R11,R12,R21,R22]=RicattiCoeff(OCMATLSC.Q0,J,OCMATLSC.subspacedim);
