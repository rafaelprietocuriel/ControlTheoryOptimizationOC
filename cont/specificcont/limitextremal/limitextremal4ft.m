...
if OCMATLSC.targettype==3
    modelpar(OCMATLSC.parameterindex)=freepar(OCMATLSC.parametercoord);
end
if OCMATLSC.freeparametervector
    if isempty(OCMATLSC.freeinitialparametervalue)
        modelpar(OCMATLSC.freeparameterindex)=freepar(OCMATLSC.freeparametervectorcoordinate);
    else
        modelpar(OCMATLSC.freeparameterindex)=OCMATLSC.freeinitialparametervalue+freepar(OCMATLSC.freeparametervectorcoordinate)*OCMATLSC.freeparameterdirection;
    end
end

if OCMATLSC.targettype==2 || OCMATLSC.optimalhorizon
    arctime=[OCMATLSC.initialtime freepar(OCMATLSC.switchtimecoord).' freepar(OCMATLSC.endtimecoord)];
else
    arctime=[OCMATLSC.initialtime freepar(OCMATLSC.switchtimecoord).' OCMATLSC.endtime];
end
...
J=dtds*J;
if OCMATLSC.objectivevaluecalc
    J=[J;dtds*OCMATLSC.objectivefunctionjacobian(t,depvar,modelpar,arcarg)];
end
if OCMATLSC.exogenousfunction
    J=[J;dtds*OCMATLSC.exogenousjacobian(t,depvar,modelpar,arcarg)];
end
J=[J,OCMATLSC.Jext];
Jpar=OCMATLSC.Jpar;
if OCMATCONT.HE.numarc>1
    ...
    if arc<OCMATCONT.HE.numarc
        ...
        if OCMATLSC.targettype==2 || OCMATLSC.optimalhorizon
            ...
            Jpar(:,OCMATLSC.endtimecoord)=dxdt;
        end
    end
else
    if OCMATLSC.targettype==2 || OCMATLSC.optimalhorizon
...
if OCMATLSC.targettype==3
    Jmodelpar=dtds*OCMATLSC.canonicalsystemparameterjacobian(t,depvar,modelpar,arcarg);
    Jmodelpar=Jmodelpar(:,OCMATLSC.parameterindex);
    if OCMATLSC.objectivevaluecalc
        tmpval=dtds*OCMATLSC.objectivefunctionparameterjacobian(t,depvar,modelpar,arcarg);
        Jmodelpar(OCMATLSC.objectivevaluecoord,:)=tmpval(:,OCMATLSC.parameterindex);
    end
    Jpar(:,OCMATLSC.parametercoord)=Jmodelpar;
end
if ~isempty(OCMATLSC.freeparametervector)
    Jmodelpar=dtds*OCMATLSC.canonicalsystemparameterjacobian(t,depvar,modelpar,arcarg);
    if isempty(OCMATLSC.freeinitialparametervalue)
        freeJmodelpar=Jmodelpar(:,OCMATLSC.freeparameterindex);
    else
        freeJmodelpar=sum(repmat(OCMATLSC.freeparameterdirection,OCMATLSC.statecostatecoord(end),1).*Jmodelpar(:,OCMATLSC.freeparameterindex),2);
    end
    Jpar(OCMATLSC.statecostatecoord,OCMATLSC.freeparametervectorcoordinate)=freeJmodelpar;
    if OCMATLSC.objectivevaluecalc
        Jobjmodelpar=dtds*OCMATLSC.objectivefunctionparameterjacobian(t,depvar,modelpar,arcarg);
        if isempty(OCMATLSC.freeinitialparametervalue)
            freeJobjmodelpar=Jobjmodelpar(:,OCMATLSC.freeparameterindex);
        else
            freeJobjmodelpar=sum(OCMATLSC.freeparameterdirection.*Jobjmodelpar(:,OCMATLSC.freeparameterindex),2);
        end
        Jpar(OCMATLSC.objectivevaluecoord,OCMATLSC.freeparametervectorcoordinate)=freeJobjmodelpar;
    end
    if OCMATLSC.exogenousfunction
        Jexfmodelpar=dtds*OCMATLSC.exogenousparameterjacobian(t,depvar,modelpar,arcarg);
        if isempty(OCMATLSC.freeinitialparametervalue)
            freeJexjmodelpar=Jexfmodelpar(:,OCMATLSC.freeparameterindex);
        else
            freeJexjmodelpar=sum(OCMATLSC.freeparameterdirection.*Jexfmodelpar(:,OCMATLSC.freeparameterindex),2);
        end
        Jpar(OCMATLSC.exogenousdynamicscoordinate,OCMATLSC.freeparametervectorcoordinate)=freeJexjmodelpar;
    end
end
...
if OCMATLSC.targettype==3
    DJpar=dtds*OCMATLSC.canonicalsystemparameterhessian(t,depvar,modelpar,arcarg);
    Hypar(:,:,OCMATLSC.parametercoord)=DJpar(:,:,OCMATLSC.parameterindex);
end
...
resinit=[];
if OCMATLSC.targettype==2 || OCMATLSC.optimalhorizon
    timehorizon=freepar(OCMATLSC.endtimecoord);
else
    timehorizon=OCMATLSC.endtime;
end

initialstate=OCMATLSC.startvalue;
if ~isempty(OCMATLSC.fixinitstatecoord)
    resinit=OCMATLSC.bcinitial(depvara,OCMATLSC.fixinitstatecoord,OCMATLSC.initstate,modelpar,OCMATCONT.HE.arcarg(1));
end
...
if ~isempty(OCMATLSC.freevectorcoord)
    resinit=depvara(OCMATLSC.statecoord,1)-initialstate;
end
if OCMATLSC.stateconstraint
    jumparg=freepar(OCMATLSC.entrytimecoordinate);
end

if OCMATLSC.transversalityconditioncs
    restrans=OCMATLSC.bctransversalitysc(timehorizon,depvarb,jumparg(end),modelpar,OCMATLSC.jumpid(end));
else
    restrans=OCMATLSC.bctransversality(timehorizon,depvarb,modelpar,OCMATCONT.HE.arcarg(end));
end
if ~isempty(OCMATLSC.fixendstatecoord)
    restrans(OCMATLSC.fixendstatecoord,1)=depvarb(OCMATLSC.fixendstatecoord,end)-OCMATLSC.endstate;
end
if OCMATLSC.objectivevaluecalc
    OVal=OCMATLSC.salvagevalue(timehorizon,depvarb(:,end),modelpar,OCMATCONT.HE.arcarg(end));
    resinit=[resinit;depvara(OCMATLSC.objectivevaluecoord,1)-OVal];
end
if OCMATLSC.optimalhorizon
    restrans=[restrans; ...
        OCMATLSC.bcoptimalhorizon(timehorizon,depvarb,modelpar,OCMATCONT.HE.arcarg(end))];
end

for ii=1:numel(OCMATCONT.HE.arcarg)-1
    if OCMATLSC.stateconstraint && OCMATLSC.entryindex(ii)
        resconnec=[resconnec; ...
            OCMATLSC.bcstateconstraint(depvara,depvarb,modelpar,jumparg(OCMATLSC.entryindex(ii)),switchtimes,OCMATLSC.jumpid(ii),OCMATCONT.HE.edge,ii); ...
    ...
    if OCMATLSC.objectivevaluecalc
        resconnec=[resconnec; ...
            depvarb(OCMATLSC.objectivevaluecoord,ii)-depvara(OCMATLSC.objectivevaluecoord,ii+1)];
    end
end
...
h=OCMATLSC.plotcontinuation(t,sol.y,modelpar,OCMATCONT.HE.arcarg,freepar,tangent);
...
%fprintf(1,'           NF parameter: %g\n',calchnf_LP(tmesh,coeff,tangent,@operatoreq,OCMATLSC.LAE_phi,OCMATLSC.LAE_psi));
%---------------------------------------------------------------
...
for ii=id
    switch ii
        case 1
            if OCMATLSC.targettype==1
                out=OCMATLSC.targetvalue-y(OCMATLSC.targetcoordinate,1);
            elseif OCMATLSC.targettype==2
                out=OCMATLSC.targetvalue-freepar(OCMATLSC.endtimecoord);
            elseif OCMATLSC.targettype==3
                if isempty(OCMATLSC.targetcoordinate)
                    out=OCMATLSC.targetvalue-freepar(OCMATLSC.parametercoord);
                else
                    out=OCMATLSC.targetvalue-y(OCMATLSC.targetcoordinate,1);
                end
            end
...
out.solverinfo.objectivevaluecalc=OCMATLSC.objectivevaluecalc;
out.solverinfo.objectivevaluecoord=OCMATLSC.objectivevaluecoord;
out.solverinfo.conttype='limitextremal4ft';
if OCMATLSC.targettype==3
    out.solverinfo.parametercoord=OCMATLSC.parametercoord;
end
...
out.solverinfo.stateconstraint=OCMATLSC.stateconstraint;
if OCMATLSC.stateconstraint
    out.solverinfo.entrytimecoordinate=OCMATLSC.entrytimecoordinate;
    out.solverinfo.entryindex=OCMATLSC.entryindex;
    out.solverinfo.jumpid=OCMATLSC.jumpid;
end
if OCMATLSC.exogenousfunction
    out.solverinfo.exogenousdynamicscoordinate=OCMATLSC.exogenousdynamicscoordinate;
end
switch OCMATCONT.bvpmethod
    case 'bvp6c'
...
try
    if contnum==1
        save([OCMATLSC.basicglobalvarfilename '4limitextremal4ft'],'MODELINFO')
    end
    save([OCMATLSC.basicresultfilename '4limitextremal4ft'],'sout','bvpout')
catch
    failed=1;
end
