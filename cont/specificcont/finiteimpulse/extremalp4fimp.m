...
modelpar(IOCMATFTE.varyparameterindex)=freepar(OCMATCONT.HE.numparameter);
...
Jpar(:,OCMATCONT.HE.numparameter)=Jmodelpar(:,IOCMATFTE.varyparameterindex);

% if ~all(isfinite([J(:);Jpar(:)]))
%     J
% end
%-------------------------------------------------------------------------
    ...
    switch ii
        case 1
            if IOCMATFTE.findoptimalparameter
                ...
                tangent=tangent/tpar;
                tangent=tangent(OCMATCONT.HE.DDATA.meshvalcoord);
                out=tangent(IOCMATFTE.objectivevaluecoord,end);
                IOCMATFTE.derivative=out;
            else
...
if IOCMATFTE.objectivevaluecalc
    out.y=[[freepar(IOCMATFTE.initialdepvarcoord);0] out.y [freepar(IOCMATFTE.enddepvarcoord);out.y(IOCMATFTE.objectivevaluecoord,end)]];
else
...
out.solverinfo.conttype='extremalp4fimp';
...
out.solverinfo.varyparameterindex=IOCMATFTE.varyparameterindex;
out.solverinfo.contparametercoord=OCMATCONT.HE.contparametercoord;
out.solverinfo.findoptimalparameter=IOCMATFTE.findoptimalparameter;
...
switch id
    ...
    case -1
        ...
        tangent=tangent/tpar;
        tangent=tangent(OCMATCONT.HE.DDATA.meshvalcoord);
        s.data.dOdp=tangent(IOCMATFTE.objectivevaluecoord,end);
        J=frechetder(tmesh,coeff,tangent,OCMATCONT.ode,OCMATCONT.bc,OCMATCONT.icfun,OCMATCONT.odejac,OCMATCONT.bcjac,OCMATCONT.icfunjac);
        [Q,R,E]=qr(full(J)); % see Govaerts et al 2005 (p. 242)
        R(end,end)=0;R(end,end-1)=0;
        p=E*[R(1:end-1,1:end-1)\-R(1:end-1,end);1];
        s.data.phi=p'/norm(p);
        s.data.phi=s.data.phi(:).';
end
failed=0;
%------------------------------------------------------------
...
modelpar(IOCMATFTE.varyparameterindex)=coeff(OCMATCONT.HE.contparametercoord);
...
try
    if contnum==1
        save([IOCMATFTE.basicglobalvarfilename '4extremalp4fimp'],'MODELINFO')
    end
    save([IOCMATFTE.basicresultfilename '4extremalp4fimp'],'sout','bvpout')
catch
    failed=1;
end
