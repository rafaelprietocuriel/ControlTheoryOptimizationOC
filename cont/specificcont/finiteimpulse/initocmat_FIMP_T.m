function sol= initocmat_FIMP_T(ocObj,hocTrj,timeindex,varargin)
%
%
% INITOCMAT_FIMP_T
%
...
if findoptimaljumptime
    hocTrj.jumparg(findoptimaljumptime)=-1;
end
...
if isempty(timeindex)
    timeindex=length(arctime);
end
%fixedtimeidx=find(jumparg<0);
switchtimeidx=[find(jumparg>0 | [-1 jumparg(2:end-1) -1]==0) timeindex];
...
IOCMATFTE.switchtimeidx=switchtimeidx; % position in relation to arcinterval
...
IOCMATFTE.varyparameterindex=timeindex;
...
if objectivevaluecalc && length(sol.y(:,1))==2*statenum(ocObj) || objectivevaluecalc>1
    ...
    sol.y(2*statenum(ocObj)+1,:)=OT+[0 cumsum((o(1:end-1)+o(2:end))/2.*diff(time(hocTrj,1)))];
end
