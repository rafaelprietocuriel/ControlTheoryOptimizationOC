...
if OCMATPS.varytimeindex==1
    t0=freepar(OCMATCONT.HE.numparameter);
else
    t0=OCMATPS.initialtime;
end
arctime=[t0 freepar(OCMATPS.switchtimecoord).' t0+period];
...
for arc=1:OCMATCONT.HE.numarc
    ...
    arctime=[OCMATPS.initialtime freepar(OCMATPS.switchtimecoord).' period];
    diffarctime=diff(arctime);
    arcindex=OCMATCONT.HE.arcindex(arc);
    arcarg=OCMATCONT.HE.arcarg(arc);
...
for ii=id
    switch ii
        case 1
            out=OCMATPS.targettime-coeff(OCMATCONT.HE.contparametercoord);
        otherwise
            out=[];
            failed=1;
    end
...
out.octrajectory.arcinterval=[t0 freepar(OCMATPS.switchtimecoord).' t0+period];
out.octrajectory.arcarg=OCMATCONT.HE.arcarg;
out.octrajectory.x0=t0;
out.octrajectory.timehorizon=OCMATPS.period+t0;
...
out.octrajectory.solverinfo.varytimeindex=OCMATPS.varytimeindex;
...
try
    if contnum==1
        save([OCMATPS.basicglobalvarfilename '4periodicsol_T'],'MODELINFO')
    end
    save([OCMATPS.basicresultfilename '4periodicsol_T'],'sout','bvpout')
catch
    failed=1;
end
