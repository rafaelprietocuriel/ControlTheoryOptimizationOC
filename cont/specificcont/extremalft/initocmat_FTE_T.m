...
if isimplicit(ocObj)
    sol=initocmat_FTE4implicit(ocObj,ocTrj,'time','fixinitstate',initialcoordinate,'timeindex',timeindex,'targetvalue',targetvalue,varargin{:});
    return
end
...
hitfunctionidx=find(strcmpi(varargin,'hitfunction'));
...
if ~isempty(hitfunctionidx)
    hitfunction=varargin{hitfunctionidx+1};
end
...
OCMATFTE.hitfunction=hitfunction;
...
if OCMATFTE.hitfunction
    OCMATFTE.targetfunction=funch{4};
end
...
if ~isempty(userbc)
    if ischar(userbc)
        userbc=str2func(userbc);
    elseif ~isa(userbc,'function_handle')
        ocmatmsg('User function has to be function handle or function name')
        userbc=0;
    end
    if isa(userbc,'function_handle')
        OCMATFTE.userbcfunc=userbc;
    end
end
...
if timeindex>1
    fixtimeindex=unique([1 fixtimeindex]);
else
    fixtimeindex=unique(fixtimeindex);
end
arctimeindex=1:length(arctimes);
OCMATFTE.fixtimeindex=fixtimeindex;
freetimeindex=arctimeindex;
arctimeindex(fixtimeindex)=[];
freetimeindex([fixtimeindex timeindex])=[];
%arctimeindex=[arctimeindex timeindex];
%arctimeindex(fixtimeindex)=[];
OCMATFTE.freetimeindex=freetimeindex;
OCMATFTE.continuationtimeindex=timeindex;
sol.parameters=[sol.parameters arctimes(freetimeindex)];
OCMATFTE.freetimecoord=freeparametercoordinatep1:length(sol.parameters);
sol.parameters=[sol.parameters arctimes(timeindex)];
OCMATFTE.continuationtimecoordinate=length(sol.parameters);
