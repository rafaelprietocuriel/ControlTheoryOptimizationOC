...
switch OCMATCONT.continuationtype
    case 'parameter'
        if ~isempty(OCMATFTE.targetvalue)
            freemodelparameter=OCMATFTE.startvalue+freepar(OCMATFTE.continuationcoordinate)*OCMATFTE.continuationvector;
        else
            freemodelparameter=freepar(OCMATFTE.freeparametercoordinate);
        end
        modelpar(OCMATFTE.freeparameterindex)=freemodelparameter;
    otherwise
...
if ~OCMATFTE.optimalhorizon
    if strcmp(OCMATCONT.continuationtype,'time')
        arctime=OCMATFTE.switchtimes;
        arctime(OCMATFTE.freetimeindex)=freepar(OCMATFTE.freetimecoordinate);
        %arctime(OCMATFTE.continuationtimeindex)=freepar(OCMATFTE.continuationcoordinate);
        if OCMATFTE.freeendtime
            arctime=[0 arctime freepar(OCMATFTE.endtimecoordinate)];
        else
            arctime=[0 arctime OCMATFTE.endtime];
        end
    else
        arctime(OCMATFTE.freetimeindex)=freepar(OCMATFTE.freetimecoordinate);
        if OCMATFTE.freeendtime
    ...
    arctime=[OCMATFTE.initialtime freepar(OCMATFTE.switchtimecoordinate).'  freepar(OCMATFTE.optimalhorizoncoord)];
end
...
if OCMATFTE.objectivevaluecalc
    J(OCMATFTE.dFODXcoord1,OCMATFTE.dFODXcoord2)=dtds*OCMATFTE.objectivefunctionjacobian(t,depvar,modelpar,arcarg);
end
if OCMATFTE.exogenousfunction
...
if OCMATFTE.variationalcalculation
    ...
    if OCMATFTE.exogenousfunction
        J(OCMATFTE.dFEDXcoord1,OCMATFTE.ODEcoord)=J(OCMATFTE.dFEDXcoord1,OCMATFTE.ODEcoord)+[OCMATFTE.dFEDX dtds*OCMATFTE.exogenousjacobian4variationalargument(t,depvar,modelpar,arcarg) OCMATFTE.dFEDO OCMATFTE.dFEDE]+[vdts*OCMATFTE.exogenousvariationaldynamicsjacobian(t,depvar,modelpar,arcarg) OCMATFTE.dFEDO OCMATFTE.dFEDE];
    end
end
Jpar=OCMATFTE.Jpar;
if OCMATCONT.HE.numarc>1
    ...
    if arc<OCMATCONT.HE.numarc
        Jpar(OCMATFTE.ODEcoord,OCMATFTE.switchtimecoordinate(arc))=dxdt+diffarctime(arc)*(s-arc+1)*Jt;
        if arc>1
            Jpar(OCMATFTE.ODEcoord,OCMATFTE.switchtimecoordinate(arc-1))=-(dxdt+diffarctime(arc)*(s-arc)*Jt);
        end
    else
        Jpar(OCMATFTE.ODEcoord,OCMATFTE.switchtimecoordinate(arc-1))=-(dxdt+diffarctime(arc)*(s-arc)*Jt);
        if OCMATFTE.optimalhorizon
...
switchtimes=freepar(OCMATFTE.freetimecoordinate);
if ~OCMATFTE.optimalhorizon
    if OCMATFTE.freeendtime
        timehorizon=freepar(OCMATFTE.endtimecoordinate);
    else
        timehorizon=OCMATFTE.endtime;
    end
else
...
switch OCMATCONT.continuationtype
    case 'parameter'
        ...
        initialstate=OCMATFTE.initialstate(OCMATFTE.initialstatecoordinate);
        initialstatecoordinate=OCMATFTE.initialstatecoordinate;
    case 'initialstate'
        if 1%isempty(OCMATFTE.freevector)
            initialstate=OCMATFTE.startvalue+freepar(end)*OCMATFTE.continuationvector;
        else
            initialstate=OCMATFTE.startvalue;
            for ii=1:length(OCMATFTE.freevectorcoord)
                initialstate=initialstate+freepar(OCMATFTE.freevectorcoord(ii))*OCMATFTE.freevector(:,ii);
            end
        end
        initialstatecoordinate=OCMATFTE.initialstatecoordinate;
    case 'time'
        switchtimes(OCMATFTE.continuationtimeindex)=freepar(OCMATFTE.continuationcoordinate);
        initialstate=OCMATFTE.initialstate(OCMATFTE.initialstatecoordinate);
        initialstatecoordinate=OCMATFTE.initialstatecoordinate;
end
if OCMATFTE.freeparameter
...
%     if isempty(OCMATFTE.freevector)
%         initialstate=OCMATFTE.startvalue+freepar(end)*OCMATFTE.continuationvector;
%     else
%         initialstate=OCMATFTE.startvalue;
%         for ii=1:length(OCMATFTE.freevectorcoord)
...
if ~isempty(initialstatecoordinate)
    resinit=[resinit; ...
        OCMATFTE.bcinitial(depvara(:,1),initialstatecoordinate,initialstate,modelpar,OCMATCONT.HE.arcarg(1))];
    if OCMATFTE.variationalcalculation
        resinit=[resinit; ...
            OCMATFTE.variationalbcinitial(depvara,[],[],modelpar,OCMATCONT.HE.arcarg(1));];
    end
...
%     resinit(OCMATFTE.fixinitstatecoord,1)=depvara(OCMATFTE.fixinitstatecoord,1)-OCMATFTE.initstate;
% end
% if ~OCMATFTE.continitstate
%     endstate=OCMATFTE.startvalue+freepar(end)*OCMATFTE.continuationvector;
...
switchtimes=freepar(OCMATFTE.switchtimecoordinate);
if OCMATFTE.stateconstraint
...
if isempty(OCMATFTE.hitvalue)
    ...
    [t,y,z,freepar,modelpar]=drearr(tmesh,coeff);
    if ~OCMATFTE.optimalhorizon
        arctime=[OCMATFTE.initialtime freepar(OCMATFTE.switchtimecoordinate).' OCMATFTE.endtime];
    else
        arctime=[OCMATFTE.initialtime freepar(OCMATFTE.switchtimecoordinate).'  freepar(OCMATFTE.optimalhorizoncoord)];
    end
    fprintf(1,' distance hit function: %g\n',OCMATFTE.hitvaluefunc(arctime,y,modelpar,OCMATCONT.HE.arcarg));
end
    ...
    switch ii
        case 1
            if isempty(OCMATFTE.hitvalue)
                ...
                out=OCMATFTE.hitvaluefunc(arctime,y,modelpar,OCMATCONT.HE.arcarg);
            end
...
%out.arcinterval=[OCMATFTE.initialtime freepar(OCMATFTE.switchtimecoordinate).' OCMATFTE.endtime];
...
out.solverinfo.freetimecoordinate=OCMATFTE.freetimecoordinate;
