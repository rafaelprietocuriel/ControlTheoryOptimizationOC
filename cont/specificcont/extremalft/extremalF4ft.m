...
res=[resinit;resend;resconnec;restrans];
...
for arc=1:OCMATCONT.HE.numarc
    ...
    if ~OCBVP.multiarccalc
        ...
        [constr labelS]=OCMATFTE.testadmissibility(t,sol.y(eqcoord,leftarcindex(arc):rightarcindex(arc)),modelpar,arcarg);
    end
...
for ii=id
    switch ii
        case 1
            if isempty(OCMATFTE.targetdistance)
                out=OCMATCONT.OPTIONS.totalrelativedistance-coeff(OCMATCONT.HE.contparametercoord);
            else
                out=OCMATFTE.targetdistance-coeff(OCMATCONT.HE.contparametercoord);
            end
...
out.solverinfo.conttype='extremalF4ft';
...
try
    if contnum==1
        save([OCMATFTE.basicglobalvarfilename '4extremalF4ft'],'MODELINFO')
    end
    save([OCMATFTE.basicresultfilename '4extremalF4ft'],'sout','bvpout')
catch
    failed=1;
end
