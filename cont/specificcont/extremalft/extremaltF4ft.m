function out=extremaltF4ft()
% additonally calculating the fundamental solution of the linearization
out{1}=@operatoreq;
out{2}=@frechetder;
out{3}=@options;
out{4}{1}=@ode;
...
out.solverinfo.conttype='extremaltF4ft';
...
try
    if contnum==1
        save([OCMATFTE.basicglobalvarfilename '4extremaltF4ft'],'MODELINFO')
    end
    save([OCMATFTE.basicresultfilename '4extremaltF4ft'],'sout','bvpout')
catch
    failed=1;
end
