...
if OCMATCONT.HE.numarc>1
    ...
    Jpar(:,OCMATFTE.switchtimecoord)=dxdt+diffarctime(arc)*(s-arc)*Jt;
end
...
if OCMATFTE.transversalityconditioncs
    restrans=OCMATFTE.bctransversalitysc(timehorizon,depvarb,jumparg(end),modelpar,OCMATCONT.HE.arcarg(end));
else
...
if OCMATFTE.findoptimalhorizon
    XT=y(:,end);
    T=freepar(OCMATFTE.switchtimecoord(end));
...
for ii=id
    switch ii
        case 1
            ...
            elseif ~isempty(OCMATFTE.hitstatevalue)
                out=y(OCMATFTE.hitstatecoordinate,1)-OCMATFTE.hitstatevalue;
            else
