...
OCMATFTE.arcinterval=arcinterval(ocgTrj);
depvar=dependentvar(ocgTrj);
...
switch continuationtype
    case 'initialstate'
        ...
        if isempty(fixtimeindex)
            % the initial and endtime are kept fixed by default
            fixtimeindex=[1 length(arcintv)];
        end
        OCMATFTE.freetimeindex=setdiff(1:length(arcintv),fixtimeindex);

        parametercoordinate=length(sol.parameters);
        sol.parameters=OCMATFTE.arcinterval(OCMATFTE.freetimeindex);
        ...
        OCMATFTE.continuationcoordinate=continuationindex;
        OCMATFTE.startvalue=depvar{1}(OCMATFTE.continuationcoordinate,1);
    ...
    case 'parameter'
        ...
        if isempty(fixinitstate)
            OCMATFTE.fixinitialstatecoordinate=statecoord(ocObj);
        else
            OCMATFTE.fixinitialstatecoordinate=fixinitstate;
        end
        if isempty(fixtimeindex)
            % the initial and endtime are kept fixed by default
            fixtimeindex=[1 length(arcintv)];
        ...
        OCMATFTE.continuationparameterindex=parameterindex(ocObj,continuationindex);
    ...
    case 'time'
        ...
        if isempty(continuationtime)
            continuationtime=length(arcintv);
        end
        if continuationtime==length(arcintv)
            OCMATFTE.freetimeindex=setdiff(1:length(arcintv),fixtimeindex);

        else
            OCMATFTE.freetimeindex=setdiff(1:length(arcintv),[fixtimeindex continuationtime]);
        end
