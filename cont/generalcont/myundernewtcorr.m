function [x,v,i] = myundernewtcorr(x0, v0)
%
% [x,v,i] = myundernewtcorr(x0, v0)
% Internal function to perform Newton correction iterations for an
...
for i = 1:cds.options.MaxCorrIters
    ...
    for t=1:2
        ...
        if norm(dx) < cds.options.VarTolerance && norm(Q) < cds.options.FunTolerance
            v = pinv([cjac(cds.curve_func,cds.curve_jacobian,x,[]);v'])*R;
            v = v/norm(v);
            return;
        end
    end
