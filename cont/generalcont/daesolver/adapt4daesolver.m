...
[sol,tmeshidx]=init_mesh(sol);

OCMATCONT.MESHDATA.tmeshidx=tmeshidx;
[coeff,xcol]=init_coefficients(sol);
if isfield(sol.data,'coeff') && length(sol.data.coeff)==length(coeff)
    % use existing coeff
    coeff=sol.data.coeff;
end
%OCMATCONT.MESHDATA.collocationtmesh=tfine;
%OCMATCONT.MESHDATA.tmesh=sol.xnew;

init_jacobian(sol);

fn=fieldnames(OCMATCONT.TMPDATA);
for ii=1:length(fn)
    OCMATCONT.MESHDATA.(fn{ii})=OCMATCONT.TMPDATA.(fn{ii});
end
OCMATCONT=rmfield(OCMATCONT,fn);
...
fn=fieldnames(OCMATCONT.TMPDATA);
for ii=1:length(fn)
    OCMATCONT.MESHDATA1_2.(fn{ii})=OCMATCONT.TMPDATA.(fn{ii});
end
OCMATCONT=rmfield(OCMATCONT,fn);
OCMATCONT=rmfield(OCMATCONT,'TMPDATA');
...
for ii=1:OCMATCONT.CollocationNumber
    psi(ii,1:OCMATCONT.CollocationNumber+1)=Psi(ii,rho,1);
end
...
function [sol,tmeshidx]=init_mesh(sol)
global OCMATCONT
...
if isfield(sol,'arcposition')
    ...
    arcpositiondata=[1;length(sol.x)];
    arcposition=[1;length(sol.xnew)];
end
...
%h=sol.xnew(2:end)-sol.xnew(1:end-1);
...
%H=repmat(h,OCMATCONT.firstordercomponentnumber,1);
...
if ~isfield(OCMATCONT,'arcnumber')
    OCMATCONT.arcnumber=arcnumber;
    OCMATCONT.arcpositiondata=arcpositiondata;
end

[xcol,tmeshidx]=makecollocationmesh(sol.xnew);
sol.data.xcol=xcol;
