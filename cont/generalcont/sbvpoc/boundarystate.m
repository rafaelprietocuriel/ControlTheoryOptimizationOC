function [yl,yr,diffmeshatswitch]=boundarystate(tmesh,y,z)
...
for arc=1:OCMATCONT.HE.numarc
    ...
    yl(odecoord,arc)=y(odecoord,1,idx(1));
...
%                 end
    if ~isempty(aecoord)
        %yl(aecoord,arc)=sum(z(aecoord,numcolscoord,idx(1)).*domainddata(arcindex).psival0(ones(numae,1),:,numcols+2),2);
                        yl(aecoord,arc)=evalcollpoly(idx(1),diffmesh,y,z,arc,0,1);
    ...
    yr(odecoord,arc)=y(odecoord,1,idx(2))+sum(diffmesh(rightarcindex(arc)-1)*z(odecoord,numcolscoord,idx(2)).*domainddata(arcindex).psival(ones(numode,1),:,numcols+1),2);
%                yr(odecoord,arc)=evalcollpoly(idx(2),diffmesh,y,z,arc,1,0);
...
%                 end
    if ~isempty(domainddata(arcindex).aecoord)
        yr(aecoord,arc)=sum(z(aecoord,numcolscoord,idx(2)).*domainddata(arcindex).psival0(ones(numae,1),:,numcols+1),2);
        yr(domainddata(arcindex).aecoord,arc)=evalcollpoly(idx(2),diffmesh,y,z,arc,1,1);
