function [tmesh,coeff,tangent,iter]=newtcorr4bvpsimple(tmesh0,coeff0,tangent0)
...
for iter=1:OCMATCONT.OPTIONS.maxnewtiter
    ...
    if OCMATCONT.OPTIONS.continuationmethod==1 % Moore Penrose
        ...
        D=DPHI\(scalMatrix*[PHI EN] );
        coeff_del=D(:,1);
        tangent=D(:,2);
        tangent=tangent/norm(tangent);
    else % Arclength
        coeff_del = DPHI\(scalMatrix*PHI);
    end
    ...
    for probe = 1:OCMATCONT.OPTIONS.maxprobes
        ...
        coeff_new_dist = norm(DPHI\( scalMatrix * PHI ));
