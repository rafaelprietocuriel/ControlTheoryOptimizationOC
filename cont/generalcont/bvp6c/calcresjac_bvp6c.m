function Jac=calcresjac_bvp6c(x,y,freepar,modelpar,ode,bc,icfun,odejac,bcjac,icfunjac)
...
for region = 1:OCBVP.numarc
    ...
    if OCBVP.numic
        icolsend=icolsstart+(OCBVP.Nint(region)+1)*OCBVP.numode;
        [Jicfun,dICdpar_i] = icfunjac(xreg,yreg,FcnArgs{:});
%         numJacOpt.diffvar=2;
%         numJacOpt.vectvars=[];
%         Jicfun=numjaccsd(icfun,{xreg,yreg(:),FcnArgs{:}},1,numJacOpt);
%         numJacOpt.diffvar=4;
%         numJacOpt.vectvars=[];
%         dICdpar_i=numjaccsd(icfun,{xreg,yreg,FcnArgs{:}},1,numJacOpt);
        Jac(OCBVP.nBCs+1:OCBVP.nBCs+OCBVP.numic,icolsstart+1:icolsend)=Jicfun;
        last_cols(OCBVP.nBCs+1:OCBVP.nBCs+OCBVP.numic,:)=last_cols(OCBVP.nBCs+1:OCBVP.nBCs+OCBVP.numic,:)+dICdpar_i;
        icolsstart=icolsend;
    end
    % Collocation equations
    if isempty(odejac)  % use numerical approx
        ...
        for i = 1:Nint
            ...
            if averageJac && (norm(Jip1 - Ji,1) <= 0.125*(nrmJi + nrmJip1)) && ...
                ...
                [Jip025,nFcalls025,dFdpar_ip025]=Fnumjac(ode,{xip025,yip025,FcnArgs{:}});
                [Jip05,nFcalls05,dFdpar_ip05]=Fnumjac(ode,{xip05,yip05,FcnArgs{:}});
                [Jip075,nFcalls075,dFdpar_ip075]=Fnumjac(ode,{xip075,yip075,FcnArgs{:}});
        ...
        for i = 1:Nint
            ...
            [Jip025, dFdpar_ip025] = odejac(xip025,yip025,FcnArgs{:});
            [Jip05, dFdpar_ip05] = odejac(xip05,yip05,FcnArgs{:});
            [Jip075, dFdpar_ip075] = odejac(xip075,yip075,FcnArgs{:});
