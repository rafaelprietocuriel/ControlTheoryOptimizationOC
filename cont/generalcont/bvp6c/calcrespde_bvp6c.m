function Phi = calcrespde_bvp6c(t,y,freepar,modelpar,ode,bc)
...
for region = 1:nregions
    FcnArgs{1} = region;
    tidx = Lidx(region):Ridx(region);   % mesh point index
    Nreg = length(tidx);
    treg = t(tidx);
    yreg = y(:,tidx);
    ...
    tip025 = 0.25*(3*ti + tip1);
    tip075 = 0.25*(ti + 3*tip1);
    yip025 = (54*yi + 10*yip1 + (9*Fi - 3*Fip1)*H)/64;
    yip075 = (10*yi + 54*yip1 + (3*Fi - 9*Fip1)*H)/64;
    if OCMATCONT.OPTIONS.xyvectorized
        Fip025 = ode(tip025,yip025,FcnArgs{:});
        Fip075 = ode(tip075,yip075,FcnArgs{:});
        nfcn = nfcn + 3;
    else % not vectorized
        Fip025 = zeros(n,Nint);
        Fip075 = zeros(n,Nint);
        for i = 1:Nint
            Fip025(:,i) = ode(tip025(i),yip025(:,i),FcnArgs{:});
            Fip075(:,i) = ode(tip075(i),yip075(:,i),FcnArgs{:});
        end
        nfcn = nfcn + 2*Nint;
    end
