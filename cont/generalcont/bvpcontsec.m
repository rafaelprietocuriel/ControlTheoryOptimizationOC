function sout=bvpcontsec(varargin)
%
...
while contnum <= MaxContStep && ~exitflag && ~UserStop.Stop()
    ...
    while 1 && ~UserStop.Stop()
        ...
        if ~isempty(coeff2) && ((contnum >= 2) && (tangent1'*tangent2 <= dir_check_angle))
            tangent2=coeff2-coeff1;
            tangent2=tangent2/norm(tangent2);
            oldstepwidth=stepwidth;
            secflag=1;
            stepwidth=stepwidth*0.75;
%             if contnum==2
%             stepwidth=0.75; % initialize step width
%             stepwidth_max=0.75;
%             stepwidth_min=1e-2;
%             end
        else
            if secflag==1
                stepwidth=oldstepwidth;
            end
            secflag=0;
        end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    newmesh=false;
    if (mod(contnum,MeshAdaptation)==0) || ~passed
