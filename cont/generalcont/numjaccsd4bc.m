%-----------------------------------------------------------------
function [dFdy,nfevals,nfcalls]=numjaccsd4bc(fun,Fargs,nF,szy,options)
...
if isempty(vectvar)
    for ii=1:ny                      % loop for each independent variable
        dFdy(:,ii)=imag(fun(Fargs{1:diffvar-1},reshape(ydel(:,ii),szy),Fargs{diffvar+1:end}))/h(ii);     % complex step differentiation
    end
    nfcalls = ny;                       % stats
else
