function [Sxint,Spxint] = devalbvpoc(sol,xint)
...
if ~strcmp(solver,'bvp6c')
    if ~ismember(solver,{'ode113','ode15i','ode15s','ode23','ode23s','ode23t',...
            'ode23tb','ode45','bvp4c','dde23','bvp5c','bvp6c'})
        error('MATLAB:deval:InvalidSolver',...
            'Unrecognized solver name ''%s'' in %s.solver.',solver,inputname(1));
    else
        % if bvp6c is not the solver, use existing deval (which ISNT builtin)
...
multipointBVP = (isequal(solver,'bvp4c') || isequal(solver,'bvp6c')) || isequal(solver,'bvp6cp') && any(diff(t)==0);
if multipointBVP
