function [tcolmesh,coeff,tangent,iter]=newtcorr4daetst(tcolmesh0,coeff0,tangent0)
...
new_coeff=coeff0;
coeff=coeff0;
tcolmesh=tcolmesh0; % time mesh cannot be changed during these steps, but can be set empty if not solution is detected
method1=1;
method2=1;
...
[tolfactor,method,coeff_new,U,L,G0,G,F,delta_x,simplified_delta_x,fcount,tangent1]= ...
    determineinitialmethod(tcolmesh0,coeff0,tangent0,fcount);
coeff_dist0=G0;
coeff_dist=G;
coeff_del=delta_x;
simplified_coeff_del=simplified_delta_x;
if OCMATCONT.OPTIONS.contlog
...
while 1
    ...
    switch method
        case 1
            ...
            new_coeff(coefficientindex,1)=coeff(coefficientindex,1)+coeff_del(coefficientindex,1);
            new_F=OCMATCONT.operatoreq(tcolmesh,new_coeff,tangent,OCMATCONT.dae,OCMATCONT.bc,OCMATCONT.icfun);
            ...
            if coeff_dist<(1-OCMATCONT.OPTIONS.lambdamin/2)*coeff_dist0 % Approximation improved
                tolfactor=check_tolerances(new_coeff,simplified_coeff_del);
                coeff(coefficientindex,1)=new_coeff(coefficientindex,1);         % Accept new approximation
                F=new_F;
                if OCMATCONT.OPTIONS.contlog
                    existidx=ismember(showvariable,who);
                    logstr=writelogdata(showvariable,existidx);
        ...
        case 2
            ...
            while ~accept_lambda
                ...
                new_coeff(coefficientindex,1)=coeff(coefficientindex,1)+lambda*coeff_del(coefficientindex,1);  % get new approximation

                F=OCMATCONT.operatoreq(tcolmesh,new_coeff,tangent,OCMATCONT.dae,OCMATCONT.bc,OCMATCONT.icfun);
                fcount = fcount + 1;
            ...
            if accept_lambda % a lambda has been accepted
                coeff(coefficientindex,1)=new_coeff(coefficientindex,1);    % Accept new solution
            ...
            if method== 2
                ...
                new_coeff(coefficientindex,1)=coeff(coefficientindex,1)+lambda*coeff_del(coefficientindex,1);
...
function [tolfactor,method,coeff_new,U,L,coeff_dist0,coeff_dist1,F,coeff_del0,coeff_del1,fcount,new_tangent]=determineinitialmethod(tcolmesh,coeff,tangent,fcount)
method1=1;
method2=1;
...
if tolfactor < 1
    method=method1;  % Fast Frozen Newton
    coeff_del1=[];
    coeff_dist1=[];
    return
end
...
if coeff_dist1<(1-OCMATCONT.OPTIONS.lambdamin/2)*coeff_dist0 % Approximation improved
    if coeff_dist1<OCMATCONT.OPTIONS.switchtoffnfactor*coeff_dist0         % Approximation improved significantly
        ...
        method=method2;             % -> Predictor Corrector Line Search
        if OCMATCONT.OPTIONS.contlog
            st=dbstack('-completenames');
