function dGdy=multlinear2_bvp4c(x,y,freepar,modelpar,ode,bc,odejac,bcjac,odehess,bchess,q1,q2)
...
%Hess=spalloc(OCBVP.nN+OCBVP.nparmc,(OCBVP.nN+OCBVP.npar)*(OCBVP.nN+OCBVP.npar),(OCBVP.nN+OCBVP.npar)*(2*OCBVP.numode+OCBVP.npar)*(OCBVP.nN+OCBVP.npar));  % sparse storage
%Hess=zeros(OCBVP.nN+OCBVP.nparmc,OCBVP.nN+OCBVP.npar,OCBVP.nN+OCBVP.npar);
dGdy=zeros(OCBVP.nN+OCBVP.nparmc,1);
...
for arc=1:numarc
    ...
    for ii=1:OCBVP.Nint(arc)
        ...
        q2_c=q2([cols cols + OCBVP.numode]);
        q1_r=q1(planes);
        tmp=[];
        for pp=1:OCBVP.numode
            tmp(:,pp)=PhiHess_yi(:,:,pp)*q2_c;
            %tmp(:,pp)=dGdy(planes(pp))-q1_r'*PhiHess_yi(:,:,pp)*q2_c;
        end
        dGdy(rows)=dGdy(rows)+tmp*q1_r;
        ...
        if OCBVP.nparmc>0
            ...
            for pp=1:2*OCBVP.numode
                tmp(:,pp)=PhiHessParT(:,1:OCBVP.nparmc,pp)*q2_c;
            end
            q1_r=q1(pidx);
            dGdy(rows)=dGdy(rows)+tmp*q1_r;
            ...
            tmp=[];
            for pp=1:OCBVP.nparmc
                tmp(:,pp)=D2Phidpar2(:,1:OCBVP.nparmc,pp)*q2_c;
            end
            q1_r=q1(planesparmc);
            dGdy(rows)=dGdy(rows)+tmp*q1_r;
        end
    ...
    if OCBVP.nparmc>0
        ...
        tmp=[];
        for pp=1:OCBVP.numode
            tmp(:,pp)=d2BCdyadya(:,arcidx,arcidx(pp))*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        
        q2_c=q2(colsb);
        tmp=[];
        for pp=1:OCBVP.numode
            tmp(:,pp)=d2BCdybdya(:,arcidx,arcidx(pp))*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        ...
        tmp=[];
        for pp=1:OCBVP.numode
            tmp(:,pp)=d2BCdpardya(:,1:OCBVP.nparmc,arcidx(pp))*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        ...
        tmp=[];
        for pp=1:OCBVP.numode
            tmp(:,pp)=d2BCdyadyb(:,arcidx,arcidx(pp))*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        ...
        tmp=[];
        for pp=1:OCBVP.numode
            tmp(:,pp)=d2BCdybdyb(:,arcidx,arcidx(pp))*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        ...
        tmp=[];
        for pp=1:OCBVP.numode
            tmp(:,pp)=d2BCdpardyb(:,1:OCBVP.nparmc,arcidx(pp))*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        ...
        tmp=[];
        for pp=1:OCBVP.nparmc
            tmp(:,pp)=d2BCdyadpar(:,arcidx,pp)*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        ...
        tmp=[];
        for pp=1:OCBVP.nparmc
            tmp(:,pp)=d2BCdybdpar(:,arcidx,pp)*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
        ...
        tmp=[];
        for pp=1:OCBVP.nparmc
            tmp(:,pp)=d2BCdpardpar(:,1:OCBVP.nparmc,pp)*q2_c;
        end
        dGdy(1:OCBVP.nBCs)=dGdy(1:OCBVP.nBCs)+tmp*q1_r;
...
% Hess(:,:,OCBVP.nN+(1:OCBVP.npar))=cat(2,last_planes,last_planes2);
% Hess(:,planespar,1:OCBVP.nN)=permute(last_planes,[1 3 2]);
