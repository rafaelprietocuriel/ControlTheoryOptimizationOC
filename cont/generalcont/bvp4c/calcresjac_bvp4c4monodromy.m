function [Jac,M]=calcresjac_bvp4c4monodromy(x,y,freepar,modelpar,ode,bc,icfun,odejac,bcjac,icfunjac)
%
...
if ~isfield(OCBVP,'F')
    calcres_bvp4c(x,y,freepar,modelpar,ode,bc,icfun);
end
F=OCBVP.F;
Fmid=OCBVP.Fmid;
%nfcn = 0;
...
limitcyclelidx=OCBVP.Lidx(OCBVP.limtcycleindex==1);
limitcycleridx=OCBVP.Ridx(OCBVP.limtcycleindex==1);
if ~isempty(limitcyclelidx)
    OCBVP.Hi=zeros(OCBVP.numode,OCBVP.numode,limitcycleridx(end)-limitcyclelidx(1)+1);
    OCBVP.Gi=zeros(OCBVP.numode,OCBVP.numode,limitcycleridx(end)-limitcyclelidx(1)+1);
end
