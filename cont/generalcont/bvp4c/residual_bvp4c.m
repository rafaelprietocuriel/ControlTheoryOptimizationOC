...
for arc = 1:OCBVP.numarc
    %counterstep=OCBVP.numode*(OCBVP.Ridx(arc)-OCBVP.Lidx(arc));
    %NewtResII=reshape(RHS(counter+(1:counterstep)),OCBVP.numode,[]);
    ...
    %temp =  (NewtResII * spdiags(1.5./hreg(:),0,OCBVP.Nint(arc),OCBVP.Nint(arc))) ./ ...
    %    max(abs(Fmid(1:OCBVP.numode,iidx)),thresh);
