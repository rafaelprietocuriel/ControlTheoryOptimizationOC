function dGdy=calchessianphi_bvp4c(x,y,freepar,modelpar,ode,bc,odejac,bcjac,odehess,bchess,leigvec,reigvec)
...
%     ya1=ya;
%     ya1(ii)=ya1(ii)+tol;
%     [dGdya1p,dGdyb,dGdpar] = BCnumjac(bc,ya1,yb,FcnArgs{2:end});
%     ya1=ya;
%     ya1(ii)=ya1(ii)-tol;
%     [dGdya1m,dGdyb,dGdpar] = BCnumjac(bc,ya1,yb,FcnArgs{2:end});
%     d2BCdYdyaTest(:,:,ii)=(dGdya1p-dGdya1m)/tol;
% end
...
for arc=1:numarc
    ...
    for ii=1:OCBVP.Nint(arc)
        ...
        for jj=1:OCBVP.numode
            ...
            if OCBVP.nparmcod
                D2Phidpardyi(:,:,jj)=-1/6*(-1/2*d2Fdy2_ip05mdInner_i(:,:,jj)*(dFdpar_ip1-dFdpar_i)*hi+1/2*dFdy_ip05*d2Fdpardy_i(:,:,jj)*hi+4*d2Fdy2_ip05mdInner_par(:,:,jj)+d2Fdpardy_i(:,:,jj))*hi;
...
if isempty(odehess)
    ...
    % TEST
    [d2FdYdyTest,d2FdYdparTest]=Fnumhess(ode,odejac,x,y,OCBVP.numode,OCBVP.npar,varargin{:});
...
odeArgs2 = {x,y,ode,odejac,varargin{:}};
odeVal=odejacaux(x,y,ode,odejac,varargin{:});

    [d2FdYdy,ignored,nbc]=numjaccsd(@odejacaux,odeArgs2,OCBVP.numode*(OCBVP.numode+OCBVP.npar),Hoptions);
