function Jac=calcresjac_bvp4c(x,y,freepar,modelpar,ode,bc,icfun,odejac,bcjac,icfunjac)
%
...
lastcolsidx=OCBVP.nN+(1:OCBVP.npar);

%Jac = spalloc(OCBVP.nN+OCBVP.nparmc,OCBVP.nN+OCBVP.npar,(OCBVP.nN+OCBVP.npar)*(2*OCBVP.numode+OCBVP.npar+OCBVP.numic));  % sparse storage
...
last_rows=zeros(OCBVP.numic,OCBVP.nN+OCBVP.npar); % entries from the integral constraints
    ...
    if isempty(odejac)  % use numerical approx
        ...
        if OCBVP.numic
            if isempty(icfunjac)
                numJacOpt.diffvar=2;
                numJacOpt.vectvars=[];
                [dICdyi,dICdpar_i]=numjaccsd(icfun,{xreg(1),yreg(:,1),FcnArgs{:}},1,numJacOpt);
                numJacOpt.diffvar=4;
                numJacOpt.vectvars=[];
            else
                [dICdyi,dICdpar_i] = icfunjac(xreg,yreg,FcnArgs{:});
            end
        end
        nrmJi = norm(Ji,1);
        ...
        for i = 1:OCBVP.Nint(arc)
            ...
            if OCBVP.numic
                if isempty(icfunjac)
                    numJacOpt.diffvar=2;
                    numJacOpt.vectvars=[];
                    [dICdyip1,dICdpar_ip1]=numjaccsd(icfun,{xip1,yip1,FcnArgs{:}},1,numJacOpt);
                    [dICdyip05,dICdpar_ip05]=numjaccsd(icfun,{xip05,yip05,FcnArgs{:}},1,numJacOpt);
                    numJacOpt.diffvar=4;
                    numJacOpt.vectvars=[];
                else
                    [dICdyip1,dICdpar_ip1] = icfunjac(xip1,yip1,FcnArgs{:});
                    [dICdyip05,dICdpar_ip05] = icfunjac(xip05,yip05,FcnArgs{:});
                end
                twiceGip05 = 2*Gip05;
                last_rows(:,cols)=last_rows(:,cols)+(hi/6*(dICdyi+twiceGip05*(OCBVP.In+hi/4*Ji)));
            end
            twiceJip05 = 2*Jip05;
            % assembly
            ...
            if OCBVP.numic
                last_rows(:,cols)=(hi/6*(dICdyip1+twiceGip05*(OCBVP.In-hi/4*Jip1)));
                last_rows(:,lastcolsidx)=last_rows(:,lastcolsidx)+hi/6*(dICdpar_ip1+dICdpar_i+dICdpar_ip05+dICdyip05*(dFdpar_ip1-dFdpar_i)*hi/2);
            end
...
%             if any(abs(Jip1test(:)-Jip1(:))/max(abs(Jip1(:)))>1e-1)
%                 %if any(abs(Jip05test(:)-Jip05(:))>1e-6)
%                 Jip1
%             end
%             if any(abs(dFdpar_ip1(:)-dFdpar_ip1test(:))/max(abs(dFdpar_ip1(:)))>1e-7)
%                 %if any(abs(dFdpar_ip05(:)-dFdpar_ip05test(:))>1e-6)
%                 dFdpar_ip1
%             end
%             OCBVP.Fref=Fmidreg(:,i);
...
%             if any(abs(Jip05test(:)-Jip05(:))/max(abs(Jip05(:)))>1e-1)
%                 %if any(abs(Jip05test(:)-Jip05(:))>1e-6)
%                 Jip05
%             end
%             if any(abs(dFdpar_ip05(:)-dFdpar_ip05test(:))/max(abs(dFdpar_ip05(:)))>1e-7)
%                 %if any(abs(dFdpar_ip05(:)-dFdpar_ip05test(:))>1e-6)
%                 dFdpar_ip05
%             end
            ...
            if OCBVP.numic
                last_rows(:,cols)=(hi/6*(dICdyip1+twiceGip05*(OCBVP.In-hi/4*Jip1)));
                last_rows(:,lastcolsidx)=last_rows(:,lastcolsidx)+hi/6*(dICdpar_ip1+dICdpar_i+4*dICdpar_ip05-dICdyip05*(dFdpar_ip1-dFdpar_i)*hi/2);
            end
            %             last_cols(rows,:) = -hi*dFdpar_ip05 + hi^2/12*Jip05* ...
            %                 (dFdpar_ip1-dFdpar_i);
            last_cols(rows,:) = -1/6*(dFdpar_ip1-1/2*Jip05*(dFdpar_ip1-dFdpar_i)*hi+4*dFdpar_ip05+dFdpar_i)*hi;
