        ...
        for i = 1:OCBVP.Nint(arc)
            ...
            if OCMATCONT.OPTIONS.testmodus
                ...
                if any(abs(Jip1test(:)-Jip1(:))/max(abs(Jip1(:)))>1e-6) %any(abs(Jip1test(:)-Jip1(:))>1e-5) %
                    %if any(abs(Jip05test(:)-Jip05(:))>1e-6)
                    fprintf('Significant difference in the dynamics part of the Jacobian between analytic and numerical computation.\n')
                    %Jip1
                    pause
                end
                if any(abs(dFdpar_ip1(:)-dFdpar_ip1test(:))/max(abs(dFdpar_ip1(:)))>1e-7) %any(abs(dFdpar_ip1(:)-dFdpar_ip1test(:))>1e-5) %
                    fprintf('Significant difference in the parameter part of the Jacobian between analytic and numerical computation.\n')
                    answer=input('Interrupt computation?  y/(n): ','s');
                    if isempty(answer)
                        % default value 'y'
                        answer='y';
                    end
                    if strcmpi(answer,'y')
                        fprintf('Open problem file %s.\n',[OCMATCONT.problem_func '.m'])
                        open(OCMATCONT.problem_func)
                        Jac=[];
                        return
                    end
                end
