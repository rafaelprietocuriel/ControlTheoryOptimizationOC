function Z=heunlocal(d,fode,Z0,exogpath,varargin)
...
nz = size(Z0,1); % get number of state variables from IC first dimension
Z = zeros(nz,nt); % declare the array of state variables
Z(:,i0) = Z0; % write the initial conditions of z
...
for i = dt0:d:dtf % index for array dt of time increments
    ...
    fZ = feval(fode,t(i0),Z(:,i0),exogpath(:,i0),varargin{:}); % get functions f and c
    Z_t = Z(:,i0) + dti * fZ; % get approximations of Z
    % get functions fy and fZ on the end of interval dt(i)
    fZ = feval(fode,t(i1),Z_t,exogpath(:,i1),varargin{:});
