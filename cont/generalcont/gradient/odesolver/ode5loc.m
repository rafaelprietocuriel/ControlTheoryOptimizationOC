function Y = ode5loc(d,odefun,tspan,y0,expth,varargin)
...
for i = 2:N
  ...
  F(:,1) = feval(odefun,ti,yi,expthi,varargin{:});  
  for stage = 2:nstages
    tstage = ti + C(stage-1)*hi;
    ystage = yi + F(:,1:stage-1)*(hi*A(1:stage-1,stage-1));
    expthistage = expthi + C(stage-1)*dexpthi;
    F(:,stage) = feval(odefun,tstage,ystage,expthistage,varargin{:});
  end  
  Y(:,i) = yi + F*(hi*B);
