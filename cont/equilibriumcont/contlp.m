function varargout=contlp(ocObj,ocEP,ap,opt,varargin)
%
...
[x0,v0]=ocmatinit_LP_LP(ocObj,ocEP,ap,varargin{:});
matcontpath=getocmatpath('cl_matcont');
if ~isempty(matcontpath)
    %mydir=cd;
    %cd(fullfile(matcontpath,'LimitPoint'))
    [outArgument.x,outArgument.v,outArgument.s,outArgument.h,outArgument.f]=cont(@limitpoint,x0,v0,opt.MATCONT);
    %cd(mydir)
else
    outArgument.x=[];
    outArgument.v=[];
