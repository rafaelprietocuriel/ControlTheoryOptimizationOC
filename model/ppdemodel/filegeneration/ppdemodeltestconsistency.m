function b=ppdemodeltestconsistency(ocStruct,testtype,symkernel,sectionStruct,sectiontext)
%
...
switch testtype
    case 'maininitfile'
        [mainpropnames,mandatorysectionname]=ppdemodelproperties('init');
        ...
        o=regexpi(sectiontext(sectionStruct.variable(1):sectionStruct.variable(2)),'\<state\:{2}'); % \:{2}, colon appears exactly two times
        if isempty([o{:}])
            b=1;
        ...
        % test if 'ode' or 'pde' appears in statedynamics section
        o=regexpi(sectiontext(sectionStruct.statedynamics(1):sectionStruct.statedynamics(2)),'(\<ode\:{2}|\<pde\:{2})');
        if isempty([o{:}])
            b=1;
            ocmatmsg('''ode'' and ''pde'' is missing in section Statedynamics.');
            return
        end
        ...
        % test if boundary conditions Neumann 'nm', Dirchlet 'di' or periodic 'pe' are specified
        o=regexpi(sectiontext(sectionStruct.boundarycondition(1):sectionStruct.boundarycondition(2)),'(\<nm\:{2}|\<di\:{2}|\<pe\:{2})');
        if isempty([o{:}])
            b=1;
            ocmatmsg('boundary conditions of type (generalized) Neumann ''(g)nm'', Dirichlet ''di'' or periodic ''pe'' is missing in section Objective.');
            return
        end
        
    case 'focinitfile'
        [mainpropnames,mandatorysectionname]=ppdemodelproperties('foc');
        % test if mandatory sections exist and if these sections are not
        % empty
        for name=mandatorysectionname
            if ~isfield(sectionStruct,name{1}) || diff(sectionStruct.(name{1}))<0
    ...
    case 'basicmodelstructure'
        ...
        parametervalue=retrieveppdemodelinformation(ocStruct,'parametervalue');
        timearg=retrieveppdemodelinformation(ocStruct,'time');
        spacearg=retrieveppdemodelinformation(ocStruct,'space');
        endtime=retrieveppdemodelinformation(ocStruct,'endtime');
        ...
        variablename=retrieveppdemodelinformation(ocStruct,'variablename');
        stdvariablename=ppdemodelvariables();
        
        objectiveintegrand=retrieveppdemodelinformation(ocStruct,'objectiveintegrand');
        objectiveintegranddependence=retrieveppdemodelinformation(ocStruct,'objectiveintegranddependence');
        ...
        endtimediscountratevariable=retrieveppdemodelinformation(ocStruct,'endtimediscountratevariable');
        objectivetype=retrieveppdemodelinformation(ocStruct,'objectivetype');
        parametervalueexpression=retrieveppdemodelinformation(ocStruct,'parametervalueexpression');
        ...
        for ii=1:length(statedynamicsconvection.value)
            testsym=testsymbolic(statedynamicsconvection.value{ii},symkernel);
            if ~all(testsym)
                b=1;
                ocmatmsg('Convection coefficient %d of the state dynamics is syntactically incorrect.\n',ii);
                return
            end
        end

        testsym=testsymbolic(statedynamicsdiffusion.value,symkernel);
        if ~all(testsym)
            b=1;
            ocmatmsg('Diffusion coefficient %d of the state dynamics is syntactically incorrect.\n',ii);
            return
        end
        ...
        if statenum.value~=numel(statedynamics.value)
            b=1;
            ocmatmsg('Number of states %d and number of ODEs/PDEs of the statedynamics %d are different.\n',statenum.value,numel(statedynamics.value));
            return
        end
        ...
        symbols=ocmatfindsym([statedynamicsconvection.value statedynamicsdiffusion.value statedynamics.value objectiveintegrand.value inequalitycontrolconstraint.value salvagevalue.value exogenousfunctionterm.value parametervalueexpression.value],symkernel);
        ...
        symbols(strcmp(symbols,timearg.value))=[];
        symbols(strcmp(symbols,spacearg.value))=[];
        ...
        for ii=1:costatenum.value
            if any(strcmp(symbols,costatename.value{ii}))
                b=1;
                ocmatmsg('Costate name ''%s'' appears as a variable in the model functions.\n',costatename.value{ii});
                return
            end
        end
        ...
        if ~isempty(variablename.value)
            ...
            ocmatmsg('For a list of the standard variable names type ''ppdemodelvariables()''\n');
            if interruptquestion()
                b=2;
                return
            end
        ...
        if inequalitycontrolconstraintnum.value
            for ii=1:arcnum.value
                ccomb=retrieveppdemodelinformation(ocStruct,'constraintcombination',arcidentifier.value{ii});
                % make cells
