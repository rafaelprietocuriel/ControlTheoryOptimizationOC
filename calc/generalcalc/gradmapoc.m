function [varargout] = gradmapoc(u,v,w,FGRAD,options,varargin)
...
% read options
Proj = 0;% Set control bounds to NaN if they are not specified
if ~isempty(u)
    if ~isfield(options,'LowerBoundU')
        options.LowerBoundU = NaN;
    elseif isempty(options.LowerBoundU)
        error('LowerBoundU should be nonempty array, scalar or NaN')
    else
        Proj = 1;
    end
    if ~isfield(options,'UpperBoundU')
        options.UpperBoundU = NaN;
    elseif isempty(options.UpperBoundU)
        error('UpperBoundU should be nonempty array, scalar or NaN')
    else
        Proj = 1;
    end
end
if ~isempty(v)
    if ~isfield(options,'LowerBoundV')
        options.LowerBoundV = NaN;
    elseif isempty(options.LowerBoundV)
        error('LowerBoundV should be nonempty array, scalar or NaN')
    else
        Proj = 1;
    end
    if ~isfield(options,'UpperBoundV')
        options.UpperBoundV = NaN;
    elseif isempty(options.UpperBoundV)
        error('UpperBoundV should be nonempty array, scalar or NaN')
    else
        Proj = 1;
    end
end
if ~isempty(w)
    if ~isfield(options,'LowerBoundW')
        options.LowerBoundW = NaN;
    elseif isempty(options.LowerBoundW)
        error('LowerBoundW should be nonempty array, scalar or NaN')
    else
        Proj = 1;
    end
    if ~isfield(options,'UpperBoundW')
        options.UpperBoundW = NaN;
    elseif isempty(options.UpperBoundW)
        error('UpperBoundW should be nonempty array, scalar or NaN')
    else
        Proj = 1;
    end
end
if isfield(options,'Projection')
    if Proj == 1
        warning('gradmap:ControlBounds','Control Bounds might be inefective since Projection is specified!')
    end
    if isa(options.Projection, 'function_handle')
        Proj = 4;
    elseif ischar(options.Projection)
        Proj = 3;
    else
        error('Projection: ''PositiveOrthant'',''UnitBox'', or function_handle')
    end
elseif Proj == 1
    options.Projection = @ValueBoundBox; %@UserDefinedBox;
end
...
if isfield(options,'LipschitzConst')
    if isreal(options.LipschitzConst)
        L = options.LipschitzConst;
    end
end
...
if isfield(options,'Optimization')
    if istrcmp(options.Optimization,'max')
        L = -abs(L);
    elseif istrcmp(options.Optimization,'min')
        L = abs(L);
    end
end
...
if isfield(options,'ConditionNumber')
    if isreal(options.ConditionNumber)
        ConditionNumber = options.ConditionNumber;
    end
end
if ConditionNumber < 1
    error('Condition Number of the function cannot be less than 1'); 
end
...
if OCMATCALC.SaveIntermediate
    sol.u=u;
    sol.v=v;
    sol.w=w;
    sol=repmat(sol,MaxIter,1);
end
    ...
    if DisplayGrad == 1 || DisplayVal == 1
        ...
        if DisplayGrad == 1
            if Proj % if there is a projection we display projected gradient
                ...
                if ~isempty(du)
                    fprintf('\t %16.8f',max(abs(du(:).*(u(:)~=yu(:)))));
                end
                if ~isempty(dv)
                    fprintf('\t %16.8f',max(abs(dv(:).*(v(:)~=yv(:)))));
                end
                if ~isempty(dw)
                    fprintf('\t %16.8f',max(abs(dw(:).*(w(:)~=yw(:)))));
                end
            else
                if ~isempty(du)
                    fprintf('\t %16.8f',max(abs(du(:))));
                end
                if ~isempty(dv)
                    fprintf('\t %16.8f',max(abs(dv(:))));
                end
                if ~isempty(dw)
                    fprintf('\t %16.8f',max(abs(dw(:))));
                end
            end
        end
        fprintf('\n');
        ...
        while Iter < MaxIter && impr == 0
            ...
            if ((y1 < y0) && (L > 0)) || ((y1 > y0) && (L < 0))
                ...
                if ((y2 < y1) && (L > 0)) || ((y2 > y1) && (L < 0))
                    %D=x1;
                    %a=4/3*(1/2*x1*y0+x1*y2-3/2*x1*y1)/x1^3;
                    %a=(-x2*y0+x1*y0-x1*y2+x2*y1)/x1/x2/(-x2+x1);
                    %b=-4/3*(x1^2*y2+5/4*y0*x1^2-9/4*y1*x1^2)/x1^3;
                    %b=(x1^2*y2-y0*x1^2+y0*x2^2-y1*x2^2)/x1/x2/(-x2+x1);
                    %                     c=y0;
                    %                     xx=linspace(0,2*x1);
                    %                     yy=a*xx.^2+b*xx+c;
                    %                     plot(xx,yy,'b',[0 x1 x2],[y0 y1 y2],'rx')
                    %                     figure(gcf)
                    %                     fprintf('Iteration Step : %d\n',Iter)
                    %if (x1*y2 + x2*y0 - (x1+x2)*y1 >= 0)%does not depend on sign(L)
                    if (8*y2+y0-9*y1)*sign(L)<=0 %sign of the derivative at x2 taking min/max into account / (2*a*x2+b)*sign(L)<=0
                        OFVal = y2;
                    else
                        Ubest = u;
                        Vbest = v;
                        Wbest = w;
                        %sp=x1/4*(4*y2+5*y0-9*y1)/(y0+2*y2-3*y1);
...
[varargout{1:nargout(FGRAD)}] = feval(FGRAD,u,v,w,...
    struct('CalcVal',1, 'CalcGrad',1, 'DisplayVal',0),varargin{:});
% works with old MALTAB versions
