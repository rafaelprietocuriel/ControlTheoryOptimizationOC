function [varargout] = gradmap(u,v,w,FGRAD,options,varargin)
...
%                     fprintf('Iteration Step : %d\n',Iter)
                    ...
                    %if ((y2-y0)*2*x1^2+x2^2*(y0-y1))*sign(L)<=0 %(2*a*x2+b)*sign(L)<=0
                    if (8*y2+y0-9*y1)*sign(L)<=0 %sign of the derivative at x2 taking min/max into account / (2*a*x2+b)*sign(L)<=0  
                    %if (-x1^2*y2+y0*x1^2+y0*x2^2-y1*x2^2-2*x2*x1*y0+2*x2*x1*y2)*sign(L)<=0 %(2*a*x2+b)*sign(L)<=0
                        OFVal = y2;
                    else
                        Ubest = u;
                        Vbest = v;
