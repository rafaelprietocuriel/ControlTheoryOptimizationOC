function [y,z,q] = nlqlode1(d,fode,ic,icz,xmesh,t,fq,u,v,varargin)
%   [Y,Z,Q] = GNLQLODE1(d,FODE,IC,XMESH,TSPAN,PFUN,QFUN) solves
%   initial value problems for small systems of nonlocal quasi-linear ODEs 
%   in one space variable x and time t  
%   simultaniously with local ODEs in time t.    
%
%   Dy(x,t)/Dt = f(x,t,y(x,t),z(t),q(t))
%
%   where vector f(x,t,y(x,t),z(t),q(t)) 
%   can depend on nonlocal functions with respect to space x
%
%   Dz(t)/Dt = fz(x,t,y(x,t),z(t),q(t)) 
%
%   q(t) = fq(x,t,y(x,t),z(t),q(t))
%
...
%
%   [FY,FZ] = FODE(XMESH,T,Y,Z,Q,U,V) is a function that evaluates the quantities
...
%   for some NT >= 3. 
%  
%   S = QFUN(XMESH,T,Y,Z,U,V) is a function that evaluates the quantities
%   on with the nonlocal variable q is calculated. The input arguments are
%   XMESH is the vector. size(XMESH) = [nx], 
%   T is the scalat time
%   Y is the matrix of state variables. size(Y) = [nx,ny]
%   Z is the vector. size(Z) = [nz,1]
%   QFUN returns vector S, size(S)  = [nq,1]. 
%
...
if nargin > 6 %exist('fq')
  q0 = fq(xmesh,t(i0),y(:,:,i0),z(:,i0),u(:,:,i0),v(:,i0),varargin{:});
  if isempty(q0), nq = 0; 
  else nq = size(q0,1);
  end
else nq = 0;
...
for i = dt0:d:dtf % index for array dt of time increments
  ...
  [fy,fz] = feval(fode,xmesh,t(i0),y(:,:,i0),z(:,i0),q(:,i0),u(:,:,i0),v(:,i0),varargin{:}); % get functions f and c
  y_t = y(:,:,i0) + dti * fy; % get approximations of y
  z_t = z(:,i0) + dti * fz; % get approximations of z
  % q
  if nq ~= 0 
    q(:,i1) = feval(fq,xmesh,t(i1),y_t,z_t,u(:,:,i1),v(:,i1),varargin{:}); 
  end
  % get functions fy and fz on the end of interval dt(i)
  [fy,fz] = feval(fode,xmesh,t(i1),y_t,z_t,q(:,i1),u(:,:,i1),v(:,i1),varargin{:});
  ...
  % nonlocal variables q
  if nq ~= 0 
    q(:,i1) = fq(xmesh,t(i1),y(:,:,i1),z(:,i1),u(:,:,i1),v(:,i1),varargin{:}); 
  end
  %
