function [u,v,w,Val,du,dv,dw,y,z,p,q,xi,theta,eta,zeta] = fun_gradmap_gnlqlpdewlodeA(u,v,w,gmopt,varargin)
persistent fpq hx ht aaqvt aav BC % Y0 l_y Z0 l_z
Val = NaN;
% get options
opt = varargin{1};
if ~isfield(opt,'AbsTol'), opt.AbsTol = eps; end
if ~isfield(opt,'MaxIter'), opt.MaxIter = 100000; end
...
% if isfield(opt,'funY0Z0') % remember initial conditions from previous iteration ????
%     if isempty(Y0), Y0 = ic; else ic = Y0; end
%     if isempty(Z0), Z0 = icz; else icz = Z0; end
% end

%x = opt.xmesh;
%t = opt.tspan;
% get function for nonlocal variables. 
if isfield(opt,'funPQ'), fpq = opt.funPQ;
else fpq = @primal_fpq; % if there is no any it uses default function 
end                    % that calls opt.funH and opt.funG
% solve primal system
for i = 1:opt.MaxIter
    [y,z,p,q] = gnlqlpdewlodeA(1,opt.funF,ic,icz,opt.funPhi,opt.xmesh,opt.tspan,fpq,u,v,w,varargin{:});
    if isfield(opt,'funY0Z0')% set both Y0 and Z0 alowing for a cyclical solution
        [ic,icz] = feval(opt.funY0Z0,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:}); 
        if max(abs(ic - y(:,:,1))) < opt.AbsTol
            if isempty(icz), break
            elseif max(abs(icz - z(:,1))) < opt.AbsTol, break
            end
        end
    else break
    end
end
if i >= opt.MaxIter, warning('maximum of iterations is exided'); end

%gnlqlode1(d,fode,ic,icz,xmesh,t,fq,u,v,varargin)
% if isfield(opt,'PhaseConstr') % Phase constraints to control depending on state variables
%     [y,z] = feval(opt.PhaseConstr,opt.xmesh,opt.tspan,y,z,p,q,varargin{:});
% end
if isfield(opt,'MixConstr') % mixed constraints to control depending on state variables
    [u,v,w] = feval(opt.MixConstr,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:});
end
...
if isfield(gmopt,'CalcVal')
  if gmopt.CalcVal
   if isfield(opt,'funJ')
    Val = feval(opt.funJ,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:});
   else
    hx = diff(x);
    ht = diff(t);
    Val = 0;
    ...
    if isfield(opt,'funLta')
      if ~isempty(opt.funLta)
        Val = Val + trapzint1q(ht,trapzint2q(hx,feval(opt.funLta,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:}))); 
      end
    end
   end
...
if isfield(opt,'funl_y')
  if ~isempty(opt.funl_y)
    ...
    else ic = feval(opt.funl_y,opt.xmesh,y(:,:,end),z(:,end),p(:,:,end),q(:,end),u(:,:,end),v(:,end),w,varargin{:}); % ,z(:,end),p(:,:,end),q(:,end),w
    end
...
if isfield(opt,'funl_z')
  if ~isempty(opt.funl_z)
    ...
    else icz = feval(opt.funl_z,z(:,end),q(:,end),v(:,end),w,varargin{:}); %???????  z(:,end),q(:,end),v(:,end),w
    end
  else error('vector funl_z must be nonempty')
  end
% else error('function or vector funl_z must be given')
end
% if isfield(opt,'funl_y_z') % remember final conditions from previous iteration ????
%     if isempty(l_y), l_y = ic; else ic = l_y; end
%     if isempty(l_z), l_z = icz; else icz = l_z; end
% end
...
if isfield(opt,'fun_adjBC') % boundary condition
  if ~isempty(opt.fun_adjBC)
    if isnumeric(opt.fun_adjBC), BC = opt.fun_adjBC;
    else BC = @adjBC;
    end
  else BC = zeros(opt.Ny,1);
  end
else error('function or vector fun_adjBC should be given')
end
for i = 1:opt.MaxIter % solve the adjoint system
    %[xi,theta,eta,zeta] = gnlqlpdewlodeA(-1,@adj_fqnlpdewode,ic,icz,BC,...
    [xi,theta,eta,zeta] = gnlqlpdewlodeA(-1,@adj_fqnlodeogd,ic,icz,BC,...
        opt.xmesh,opt.tspan,@adj_fpzq,cat(2,u,y,p),cat(1,v,z,q),w,varargin{:});
    if isfield(opt,'funl_y_z')% set both l_y and l_z alowing for a cyclical solution
        [ic,icz] = feval(opt.funl_y_z,opt.xmesh,opt.tspan,y,z,p,q,xi,theta,eta,zeta,u,v,w,varargin{:}); 
        if max(abs(ic - xi(:,:,end))) < opt.AbsTol
            if isempty(icz), break
            elseif max(abs(icz - theta(:,end))) < opt.AbsTol, break
            end
        end
    else break
    end
...
if isfield(opt,'funGradHam') % calculates the gradient by user defined function
  [du,dv,dw] = feval(opt.funGradHam,opt.xmesh,opt.tspan,y,z,p,q,...
      xi,theta,eta,zeta,u,v,w,varargin{:});
...
% -------------------
function BC = adjBC(t,theta,zeta,vzq,varargin)% boundary conditions for the ajoint system
persistent z v q nv nq
  % get control w
  w = varargin{1};
  % get options
  opt = varargin{2};
  ...
  % get functions
  BC = feval(opt.fun_adjBC,t,z,q,theta,zeta,v,w,varargin{2:end});
end