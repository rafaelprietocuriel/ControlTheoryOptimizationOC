function [fxi,ftheta] = adj_fnlode(x,t,xi,theta,zeta,uy,vzq,varargin)
persistent u y z v q nx ny nz nv nq aaxyy aaxqy aaxyz aaxz dx
  % get control w
  w = varargin{1};
  % get options
  opt = varargin{2};
...
if isfield(opt,'fun_adj')
  [fxi,ftheta] = feval(opt.fun_adj,x,t,y,z,q,xi,theta,zeta,u,v,w,varargin{2:end});
else
 % fxi
 dx = diff(opt.xmesh);
 if isfield(opt,'funF_adj')
  fxi = feval(opt.funF_adj,x,t,y,z,q,xi,theta,zeta,u,v,w,varargin{2:end});
 else
  fxi = zeros(nx,ny);
  if isfield(opt,'funL_y')
   if ~isempty(opt.funL_y)
    if isnumeric(opt.funL_y), fxi = fxi + opt.funL_y;
    else fxi = fxi + feval(opt.funL_y,x,t,y,z,q,u,v,w,varargin{2:end});
    end
   end
  end
  if isfield(opt,'funF_y')
   if ~isempty(opt.funF_y) % size(funF_y) = [nx,ny,ny]
    ...
    else aaxyy(:,:,:) = feval(opt.funF_y,x,t,y,z,q,u,v,varargin{2:end});
    end
  ...
  if isfield(opt,'funH_y')
   if ~isempty(opt.funH_y) % size(funH_y) = [nx,nq,ny]
    ...
    else aaxqy(:,:,:) = feval(opt.funH_y,x,t,y,z,u,v,varargin{2:end});
    end
 ...
 if isfield(opt,'funS_adj')
  ftheta = feval(opt.funS_adj,x,t,y,z,theta,v,w,varargin{2:end});
 else
  ftheta = zeros(nz,1);
  if isfield(opt,'funS_z')
   if ~isempty(opt.funS_z) 
    ...
    else aazz(:,:) = feval(opt.funS_z,x,t,y,z,v,varargin{2:end});
    end
  ...
  if isfield(opt,'funL_z')
   if ~isempty(opt.funL_z) 
    ...
    else ftheta = ftheta + trapzint(dx,feval(opt.funL_z,x,t,y,z,v,w,varargin{2:end}));
    end
   end
  end
  if isfield(opt,'funF_z')
   if ~isempty(opt.funF_z) % size(funF_z) = [nx,ny,nz]
    ...
    else aaxyz(:,:,:) = feval(opt.funF_z,x,t,y,z,q,u,v,varargin{2:end});
    end
