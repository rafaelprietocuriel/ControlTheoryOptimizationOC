function [u,v,w,Val,du,dv,dw,y,z,p,q,xi,theta,eta,zeta] = fun_gradmap_gnlodeoc(u,v,w,gmopt,varargin)
persistent fpq hx ht aaqvt aav ic icz
Val = NaN;
du = u;
dv = v;
dw = w;
...
if isfield(opt,'funY0') % may be it is good to unite Y0 and Z0
    if ~isempty(opt.funY0)
        if isnumeric(opt.funY0)
            ic = opt.funY0;
        else
            ic = feval(opt.funY0,opt.xmesh,w,varargin{:});
        end
    else
        error('vector funY0 must be nonempty')
    end
else
    error('function or vector funY0 must be given')
end
%x = opt.xmesh;
%t = opt.tspan;
%
...
if isfield(opt,'funPQ')
    fpq = opt.funPQ;
else
    fpq = @primal_fpq; % if there is no any it uses default function
end                    % that calls opt.funH and opt.funG
% solve primal system
[y,z,p,q] = gnlode1(1,opt.funF,ic,icz,opt.xmesh,opt.tspan,fpq,u,v,w,varargin{:});
...
if isfield(opt,'funl_y') % may be it is good to unite l_y and l_z ....
    if ~isempty(opt.funl_y)
        if isnumeric(opt.funl_y)
            ic = opt.funl_y;
        else
            ic = feval(opt.funl_y,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:});
        end
    else
        error('vector funl_y must be nonempty')
    end
else
    error('function or vector funl_y must be given')
end
if isfield(opt,'funl_z')
    if ~isempty(opt.funl_z)
        if isnumeric(opt.funl_z)
            ...
            icz = feval(opt.funl_z,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:});
        end
    else
        error('vector funl_z must be nonempty')
    end
elseif isfield(opt,'funZ0')
    error('function or vector funl_z must be given')
end
% calculate derivatives of side control at the initial moment of time
%dydx = (y(opt.Nx+2,:,2) - y(opt.Nx+1,:,2)) ./ (z(1:size(y,2),2) - z(1:size(y,2),1));
...
[xi,theta,eta,zeta] = gnlode1(-1,@adj_gnlode,ic,icz,...
...
elseif isfield(opt,'funGradHamUV') % calculates the gradient by user defined function
    nt = opt.Nt;
    nx = opt.Nt; % ???????????? may be nx = opt.Nx;
    du = zeros(size(u));
    dv = zeros(size(v));
    for it = 1:nt
        n = nx+it;
        [du(1:n,:,it),dv(:,it)] = feval(opt.funGradHamUV,...
            [opt.xmesh;z(1:size(opt.xmesh,2),1:end)'],opt.tspan(it),y(1:n,:,it),z(:,it),p(1:n,:,it),...
            q(:,it),xi(1:n,:,it),theta(:,it),eta(1:n,:,it),zeta(:,it),...
            u(1:n,:,it),v(:,it),w,varargin{:});
    end
    dw = w; %!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
