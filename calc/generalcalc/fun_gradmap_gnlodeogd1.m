function [u,v,w,Val,du,dv,dw,y,z,p,q,xi,theta,eta,zeta] = fun_gradmap_gnlodeogd1(u,v,w,gmopt,varargin)
persistent fpq hx ht aaqvt aav
Val = NaN;
du = u;
dv = v;
dw = w;
...
if isfield(opt,'funY0') % good to unite it with Z0
  if ~isempty(opt.funY0)
    if isnumeric(opt.funY0), ic = opt.funY0;
    else ic = feval(opt.funY0,opt.xmesh,w,varargin{:}); 
    end
...
icz = opt.xmesh(end,:)';
% solve primal system
[y,z,p,q] = gnlodeogd1(1,opt.funF,ic,icz,opt.funPhi,opt.xmesh,opt.tspan,fpq,u,v,varargin{:});
...
if isfield(opt,'funl_y')
  if ~isempty(opt.funl_y)
    ...
    else ic = feval(opt.funl_y,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:}); 
    end
...
if isfield(opt,'funl_z')
  if ~isempty(opt.funl_z)
    ...
    else icz = feval(opt.funl_z,opt.xmesh,opt.tspan,y,z,p,q,u,v,w,varargin{:}); 
    end
...
[xi,theta,eta,zeta] = gnlodeogd1(-1,@adj_fqnlodeogd,ic,icz,[],...
    [opt.xmesh;z(1:size(opt.xmesh,2),1:end)'],opt.tspan,...
    @adj_fpzq,cat(2,u,y,p),cat(1,v,z,q),w,varargin{:});
