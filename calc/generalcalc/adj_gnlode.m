function [fxi,ftheta] = adj_gnlode(x,t,xi,theta,eta,zeta,uyp,vzq,varargin)
% adjoint system right-hand side function
% for non local ODE on growing domain
...
if isfield(opt,'fun_adj')
  %if nargout(opt.fun_adj) > 1
    [fxi,ftheta] = feval(opt.fun_adj,x,t,y,z,p,q,xi,theta,eta,zeta,u,v,w,varargin{2:end});
  %else
  %  [fxi] = feval(opt.fun_adj,x,t,y,z,p,q,xi,theta,eta,zeta,u,v,w,varargin{2:end});
  %  ftheta = [];
  %end
else
 % fxi
