function ppdeObj=changeparametervalue(ppdeObj,varargin)
%
...
% equal the number of parameter values of 'stdocmodel' OCOBJ.
% 
...
% changing one parameter would be ppdeObj=changeparametervalue(ppdeObj,1,0.05),
% for changing more parameters 
% ppdeObj=changeparametervalue(ppdeObj,[1 2 3],[0.5 1.5 4.02]). 
%
...
% ppdeObj=changeparametervalue(ppdeObj,'alpha,beta',[1.5 2.1]).
% or
% ppdeObj=changeparametervalue(ppdeObj,{'alpha','beta'},[1.5 2.1]).
...
[oldparvalue totalparname]=parametervalue(ppdeObj);
...
if ~isempty(parname)
    ...
    for ii=1:numparvalue
        ...
        if ~isempty(idx)
            ...
            ocmaterror('Parameter name ''%s'' is unknown.',parname{ii})
        end
    end
end

if length(parindex)~=numparvalue
    error('Incorrect number of parameter values.')
end
if max(parindex)>numel(oldparvalue) || min(parindex)<1
    error('Incorrect parameter index.')
end

% change old parameter values to new values
oldparvalue(parindex)=newparvalue;
ppdeObj=changeparametervalue(ppdeObj,oldparvalue);