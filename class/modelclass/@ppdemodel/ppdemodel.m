function ppdeocObj=ppdemodel(firstarg,Result)
%
% PPDEOCMODEL initializes a parabolic PDE ocmodel. This object consists of
% two the two fields MODEL and RESULT.
%
% PPDEOCMODEL() an empty object is returned
%
% PPDEOCMODEL(OCSTRUCT) 'OCSTRUCT' is a regular model structure, as e.g.
% returned by the initialization process 
%
% PPDEOCMODEL(MODELNAME) the model structure is loaded from the data file
% stored in the corresponding user data folder and generated during the
% initialization process.
%
% PPDEOCMODEL(FILENAME) the model structure is loaded from the data file
% 'FILENAME', where 'FILENAME' is either on the MATLAB path or has to
% consist of the full path.
%
% PPDEOCMODEL(.,RESULT) RESULT is a structure storing results for the
% model. 
...
ppdeocObj.Model=ocStruct;
ppdeocObj.Result=Result;
ppdeocObj=class(ppdeocObj,'ppdemodel');