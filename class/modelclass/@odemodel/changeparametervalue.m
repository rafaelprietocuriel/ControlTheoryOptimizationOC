function odeObj=changeparametervalue(odeObj,varargin)
%
...
% changing one parameter would be odeObj=changeparametervalue(odeObj,1,0.05),
% for changing more parameters 
% odeObj=changeparametervalue(odeObj,[1 2 3],[0.5 1.5 4.02]). 
%
...
% odeObj=changeparametervalue(odeObj,'alpha,beta',[1.5 2.1]).
% or
% odeObj=changeparametervalue(odeObj,{'alpha','beta'},[1.5 2.1]).
...
odeObj=cleanmodel(odeObj);
if isnumeric(varargin{1}) && nargin==2      % Second argument is a vector with the values of the new parameters
    odeObj=setparametervalue(odeObj,varargin{1});
    return
end
[oldparvalue totalparname]=parametervalue(odeObj);
if nargin==2 && isocmodel(varargin{1})
    ...
    odeObj=setparametervalue(odeObj,newparvalue);
    return
end
if nargin>=3
...
odeObj=changeparametervalue(odeObj,oldparvalue);