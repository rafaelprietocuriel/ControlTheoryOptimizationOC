...
%
% PLOT3CONT(ODEOBJ,'XDATA',XCOORD,'YDATA',YCOORD,'ZDATA',ZCOORD) plots for
% each continuation process of class 'extremal' stored in ODEOBJ the last
...
if isempty(odeObj) || isempty(resultstruct) || ~isfield(resultstruct,'Continuation')
    if nargout==1
        varargout{1}=[];
    end
    return
    ...
    if strmatch(contclass,contresult{ii}.ContinuationClassification)
        ...
        if isstruct(solObj)
            for jj=contindex
                ...
                %htmp=line(ocTrj,'xdata',xvar,'xcoord',xcoord,'ydata',yvar,'ycoord',ycoord,'zdata',zvar,'zcoord',zcoord,'ocmodel',odeObj,'Tag',[tagname '_Nr:' num2str(ii) '_' num2str(jj)],varargin{:});
                htmp=plot3(currentsolObj,xcoord,ycoord,zcoord,'xdata',xvar,'ydata',yvar,'zdata',zvar,'ocmodel',odeObj,varargin{:});
            ...
            %htmp=line(solObj,'xdata',xvar,'xcoord',xcoord,'ydata',yvar,'ycoord',ycoord,'zdata',zvar,'zcoord',zcoord,'ocmodel',odeObj,'Tag',[tagname '_Nr:' num2str(ii)],varargin{:});
            htmp=plot3(solObj,xcoord,ycoord,zcoord,'xdata',xvar,'ydata',yvar,'zdata',zvar,'ocmodel',odeObj,varargin{:});
