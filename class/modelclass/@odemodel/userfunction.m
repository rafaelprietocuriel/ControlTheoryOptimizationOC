function varargout=userfunction(odeObj,solObj,varargin)
%
...
if isempty(solObj)
    try
        varargout{1}=feval(odeObj,'UserFunction',[],[],par,[]);
    catch
        varargout{1}=[];
        return
    end
    ...
    for ii=1:arcn
        ...
        u=feval(odeObj,'UserFunction',indepvar(arcp),depvar(:,arcp),par,arcarg(ii));
        if connectflag
            varargout{1}(:,arcp)=u;
        else
            varargout{ii}=u;
