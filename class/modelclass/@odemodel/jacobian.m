function J=jacobian(odeObj,varargin)
%
...
if isempty(solstruct)
    ...
    J=feval(odeObj,'SymbolicDynamicsJacobian',arcarg);
else
    % return optimal control value evaluated at 'solstruct'
    if nargin>=4
        %indepvar=varargin{1};
    end
    J=feval(odeObj,'DynamicsJacobian',indepvar,solstruct,parametervalue(odeObj),arcarg);
    %J=feval(odeObj,'EquilibriumJacobian',indepvar,solstruct,parametervalue(odeObj),arcarg);
end
