...
%
% SAVE(OCOBJ,FN) uses the format string FORMAT (see SPRINTF for
% details) for the parameter values.
%
% SAVE(OCOBJ,FORCE) FORCE = 1 forces to overwrite an already
% existing file. Default value is 0.
%
% SAVE(OCOBJ,FN,FORCE,FN) FN provides an alternative filename.
%
...
if nargin>=2 && length(varargin)==1 && strcmpi(varargin{1},'workspace')
    workspacename=wsname(mmObj,fn);
    ...
    return
elseif nargin==2 && length(varargin{1})==1 strcmpi(varargin{1},'modeldata')
...
if isempty(savedir)
    savedir=fullocmatfile(userdatafolder(mmObj));
end
