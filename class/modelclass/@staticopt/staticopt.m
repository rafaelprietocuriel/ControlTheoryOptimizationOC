function ocObj=staticopt(firstarg,Result,varargin)
%
% STATICOPT initializes a static optimization model. This object consists of the
% two fields MODEL and RESULT. 
%
% STATICOPT() an empty object is returned
%
% STATICOPT(OCSTRUCT) 'OCSTRUCT' is a regular model structure, as e.g.
% returned by the initialization process 
%
% STATICOPT(MODELNAME) the model structure is loaded from the data file
% stored in the corresponding user data folder and generated during the
% initialization process.
%
% STATICOPT(FILENAME) the model structure is loaded from the data file
% 'FILENAME', where 'FILENAME' is either on the MATLAB path or has to
% consist of the full path.
%
% STATICOPT(.,RESULT) RESULT is a structure storing results for the model.
...
if ~isempty(ocStruct) && ~force
    if ~isstruct(ocStruct) || staticoptmodeltestconsistency(ocStruct,'basicmodelstructure',symkernel)
