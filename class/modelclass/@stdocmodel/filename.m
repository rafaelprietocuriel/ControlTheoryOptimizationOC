function fn=filename(ocObj,varargin)
%
...
if iscell(placeholderidx)
    ...
    for ii=1:numel(placeholderidxtmp)
        placeholderidx(ii)=parameterindex(ocObj,placeholderidxtmp{ii});
    end
end
if isempty(format)
    format='%3.4g';
...
% declare constants
[par,parvar]=parametervalue(ocObj);
fn=modelname(ocObj);
if isempty(idx)
    idx=1:numel(par);
end
