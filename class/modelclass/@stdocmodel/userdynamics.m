function varargout=userdynamics(ocObj,solObj,varargin)
%
...
if isempty(solObj)
    try
        varargout{1}=feval(ocObj,'ExogenousDynamics',[],[],par,[]);
    catch
        varargout{1}=[];
        return
    end
    ...
    for ii=1:arcn
        ...
        u=feval(ocObj,'ExogenousDynamics',indepvar(arcp),depvar(:,arcp),par,arcarg(ii));
        if connectflag
            varargout{1}(:,arcp)=u;
        else
            varargout{ii}=u;
