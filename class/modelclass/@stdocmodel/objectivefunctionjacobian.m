function varargout=objectivefunctionjacobian(ocObj,solObj,varargin)
%
% 
...
for ii=1:arcn
    ...
    J=zeros(equationnum,size(arcp,2));
    for jj=1:size(arcp,2)
        tmp=feval(ocObj,'ObjectiveFunctionJacobian',indepvar(arcp(jj)),depvar(:,arcp(jj)),parametervalue(ocObj),arcarg(ii));
        if size(arcp,2)>1
            J(:,jj)=tmp(:);
        else
            J=tmp;
    ...
    if connectflag
        varargout{1}(1:equationnum,arcp)=J;
    else
        varargout{ii}=J;
    end
end
