function varargout=hamiltonian(ocObj,solObj,varargin)
%
...
% consisting of the Hamiltonian values for each arc separately. 
%
...
if isempty(solObj)
    ...
    H=feval(ocObj,'SymbolicHamiltonian',arcarg,repflag);
    lm=lagrangemultiplier(ocObj);
    for ii=1:length(lm)
        H=subs(H,mystr2sym(lm{ii}),0);
    end
    varargout{1}=H;
else
    ...
    for ii=1:arcn
        ...
        H=feval(ocObj,'Hamiltonian',indepvar(arcp),depvar(:,arcp),par,arcarg(ii));
        if connectflag
            varargout{1}(arcp)=H;
        else
            varargout{ii}=H;
