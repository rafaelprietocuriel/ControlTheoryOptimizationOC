...
%         end
        ...
        if ~isconsistent(ocObj,solObj)
            ocmatmsg('Model and continuation object %d are not consistent.\n',ii)
        end
        ...
        if isstruct(solObj)
            for jj=contindex
                if strcmp(contfield,'ContinuationInformation')
                    switch contclass
                        case {'indifferencesolution','indifferencesolution4ft','heteroclinic','indifferencedistribution','indifferencesolution4ep_ft'}
                            currentsolObj=sol2multipath(solObj(jj).data.sol);
                        case {'extremal','limitextremal','limitextremal4ft','extremal4ft'}
                            currentsolObj=octrajectory(solObj(jj).data.sol);
                        case {'extremalgrad2ep'}
                            currentsolObj=ocgradtrajectory(solObj(jj).data.sol.extremal);
                    end
                else
                    switch contclass
                        case {'indifferencesolution','indifferencesolution4ft','heteroclinic','indifferencedistribution','indifferencesolution4ep_ft'}
                            currentsolObj=sol2multipath(solObj(jj));
                        case {'extremal','limitextremal','limitextremal4ft','extremal4ft','extremalt4ft','extremalp4ft'}
                            currentsolObj=octrajectory(solObj(jj));
                        case {'extremalgrad2ep'}
                            currentsolObj=ocgradtrajectory(solObj(jj));
                    end
                end
                ...
                htmp=plot(currentsolObj,xcoord,ycoord,'xdata',xvar,'ydata',yvar,'ocmodel',ocObj,varargin{:});
                if length(htmp)==1
                    set(htmp,'Tag',[contname '_' tagname '_Nr:' num2str(ii) '_' num2str(jj)])
                else
                    for kk=1:length(htmp)
                        set(htmp(kk),'Tag',[contname '_' tagname '_Nr:' num2str(ii) '_' num2str(jj) '_' num2str(kk)])
                    end
                end
                if holdinter
                    hold on
                ...
                if showtag
                    center = centroid(get(htmp,'XData'),get(htmp,'YData'));
                    htext=text(center(1),center(2),sprintf('%s', get(h(end),'Tag')));
                end
                figure(gcf)
                %pause(0.1)
            end
        else
            %htmp=line(solObj,'xdata',xvar,'xcoord',xcoord,'ydata',yvar,'ycoord',ycoord,'ocmodel',ocObj,'Tag',[tagname '_Nr:' num2str(ii)],varargin{:});
            htmp=plot(solObj,xcoord,ycoord,'xdata',xvar,'ydata',yvar,'ocmodel',ocObj,varargin{:});
            if discrete && strncmp(contclass,'extremal',8)
                htmp2=zeros(length(htmp),1);
                counter=0;
                for jj=1:length(htmp)
                    ...
                    for kk=1:length(xdata)
                        ...
                        set(htmp2(kk),'Tag',[contname '_' tagname '_Nr:' num2str(ii) '_' num2str(jj) '_Pt:' num2str(counter)])
                    end
                end
                delete(htmp)
                set(gca,'NextPlot',nextplot);
                h=[h(:);htmp2(:)];
            elseif discrete && (strncmp(contclass,'indifferencesolution',20) || strncmp(contclass,'heteroclinic',12))
                if ~strcmp(class(solObj),'occurve')
                    n=multiplicity(solObj);
                else
                    n=1;
                end
                counterh=0;
                for jj=1:n
                    ...
                    for kk=1:arcn
                        counterh=counterh+1;
                        xdata=get(htmp(counterh),'XData');
                        ydata=get(htmp(counterh),'YData');
                        clr=get(htmp(counterh),'Color');
                        nextplot=get(gca,'NextPlot');
                        set(gca,'NextPlot','add');
                        for ll=1:length(xdata)
                            counter=counter+1;
                            htmp2(ll)=plot(xdata(ll),ydata(ll),'Marker','x','Color',clr);
                            set(htmp2(ll),'Tag',[contname '_' tagname '_Nr:' num2str(ii) '_' num2str(jj) '_' num2str(kk) '_Pt:' num2str(counter)])
                        end
                    end
                end
            else
                if length(htmp)==1
                    set(htmp,'Tag',[contname '_' tagname '_Nr:' num2str(ii)])
                else
                    for jj=1:length(htmp)
                        set(htmp(jj),'Tag',[contname '_' tagname '_Nr:' num2str(ii) '_' num2str(jj)])
                    end
                end
                h=[h;htmp];
                if showtag
                    for jj=1:length(htmp)
                        pt=[get(htmp(jj),'XData');get(htmp(jj),'YData')];
                        l=cumsum(sqrt((pt(1,2:end)-pt(1,1:end-1)).^2+(pt(2,2:end)-pt(2,1:end-1)).^2));
                        [dum idx]=min(abs(l-l(end)/2));
                        tag=get(htmp(jj),'Tag');
                        htext=text(pt(1,idx),pt(2,idx),sprintf('%s',tag(strfind(tag,':')+1:end)));
                    end
                end
            end
        end
