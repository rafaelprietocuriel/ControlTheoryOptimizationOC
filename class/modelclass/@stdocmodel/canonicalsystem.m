function varargout=canonicalsystem(ocObj,solObj,varargin)
%
...
if isempty(solObj)
    ...
    try
        ...
        canonicalsystem=retrievemodelinformation(ocObj,'canonicalsystem',[],getsymkernel());
        varargout{1}=sym(cell2vectorstring(canonicalsystem.value)).';
    end
else
    ...
    for ii=1:arcn
        ...
        dxdt=feval(ocObj,'CanonicalSystem',indepvar(arcp),depvar(:,arcp),par,arcarg(ii));
        if connectflag
