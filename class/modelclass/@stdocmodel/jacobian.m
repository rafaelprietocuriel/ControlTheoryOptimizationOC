function varargout=jacobian(ocObj,varargin)
%
...
elseif isdynprimitive(solstruct) || isoctrajectory(solstruct) || isocasymptotic(solstruct) || isoccurve(solstruct)
    ...
    indepvar=time(ocObj,solstruct,1);
...
if isempty(solstruct)
    ...
    varargout{1}=feval(ocObj,'SymbolicCanonicalSystemJacobian',arcarg);
else
    ...
    if dynprimitiveflag
        try
            %varargout{1}=feval(ocObj,'CanonicalSystemJacobian',indepvar,depvar,parametervalue(ocObj),arcarg);
            if strcmp(jacobianfile,'CanonicalSystemJacobian')
                varargout{1}=feval(ocObj,jacobianfile,indepvar,depvar,parametervalue(ocObj),arcarg);
            else
                varargout{1}=feval(ocObj,jacobianfile,indepvar,depvar,parametervalue(ocObj),arcarg);
            end
        catch
        ...
        for ii=1:arcn
            ...
            for jj=1:size(arcp,2)
                tmp=feval(ocObj,'CanonicalSystemJacobian',indepvar(arcp(jj)),depvar(:,arcp(jj)),parametervalue(ocObj),arcarg(ii));
                if size(arcp,2)>1
                    J(:,jj)=tmp(:);
                else
                    J=tmp;
