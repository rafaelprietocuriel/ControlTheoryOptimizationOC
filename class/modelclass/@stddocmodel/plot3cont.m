...
if isempty(docObj) || isempty(resultstruct) || ~isfield(resultstruct,'Continuation')
    if nargout==1
        varargout{1}=[];
    end
    return
    ...
    if strmatch(contclass,contresult{ii}.ContinuationClassification)
        ...
        if isstruct(solObj)
            for jj=contindex
                ...
                %htmp=line(ocTrj,'xdata',xvar,'xcoord',xcoord,'ydata',yvar,'ycoord',ycoord,'zdata',zvar,'zcoord',zcoord,'ocmodel',docObj,'Tag',[tagname '_Nr:' num2str(ii) '_' num2str(jj)],varargin{:});
                htmp=plot3(currentsolObj,xcoord,ycoord,zcoord,'xdata',xvar,'ydata',yvar,'zdata',zvar,'ocmodel',docObj,varargin{:});
            ...
            %htmp=line(solObj,'xdata',xvar,'xcoord',xcoord,'ydata',yvar,'ycoord',ycoord,'zdata',zvar,'zcoord',zcoord,'ocmodel',docObj,'Tag',[tagname '_Nr:' num2str(ii)],varargin{:});
            htmp=plot3(solObj,xcoord,ycoord,zcoord,'xdata',xvar,'ydata',yvar,'zdata',zvar,'ocmodel',docObj,varargin{:});
