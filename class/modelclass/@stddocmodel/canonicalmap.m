function varargout=canonicalmap(ocObj,solObj,varargin)
%
...
if isempty(solObj)
    ...
    varargout{1}=feval(ocObj,'SymbolicCanonicalSystemMap',arcarg,repflag);
else
    % return value of the canonical system evaluated at 'depvar'
    for ii=1:arcn
        arcp=arcpos(1,ii):arcpos(2,ii)+1;
        equationnum=canonicalmapdimension(ocObj,arcarg(ii));
        dxdt=feval(ocObj,'CanonicalSystemMap',indepvar(arcp(2:end)),depvar(:,arcp),par,arcarg(ii));
        varargout{1}(1:equationnum,arcp(1:end-1))=dxdt;
    end
end
