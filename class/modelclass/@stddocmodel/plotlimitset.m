...
%
% PLOTLIMITSET(DOCOBJ,'XDATA',XCOORD,'YDATA',YCOORD) plots the equilibria
% stored in DOCOBJ. The values of the x/y-axis are determined by the
...
if strcmpi(limitclass,'Fixpoint')
    limitObj=fixpoint(docObj);
elseif strcmpi(limitclass,'PeriodicSolution')
    limitObj=periodicsolution(docObj);
end
...
for ii=idx
    counter=counter+1;
    solObj=limitObj{ii};
    h(counter)=plot(solObj,xcoord,ycoord,'xdata',xvar,'ydata',yvar,'ocmodel',docObj,varargin{:});
