function out=impulseobjectivejacobian(ocObj,solObj,varargin)
%
...
if isempty(solObj)
    ...
    out=feval(ocObj,'SymbolicImpulseObjectiveFunctionJacobian',jumparg,repflag);
else
    counter=0;
    for ii=1:length(jumparg)
        if jumparg(ii)
            counter=counter+1;
            out(:,counter)=feval(ocObj,'ImpulseObjectiveFunctionJacobian',arcintv(ii),jdepvar(:,2*ii-1:2*ii),par,[],jumparg(ii),varargin{:});
        end
    end
end