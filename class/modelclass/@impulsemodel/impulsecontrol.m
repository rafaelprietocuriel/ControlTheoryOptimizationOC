function varargout=impulsecontrol(ocObj,solObj,varargin)
%
...
if isempty(solObj)
    ...
    varargout{1}=feval(ocObj,'SymbolicOptimalImpulseControl',arcarg);
else
    ...
    for ii=1:length(jumparg)
        if jumparg(ii)
            counter=counter+1;
            varargout{1}(1:ctrlnum,counter)=feval(ocObj,'OptimalImpulseControl',indepvar(ii),depvar(:,2*ii-1:2*ii),par,[],jumparg(ii));
        elseif ii==1
            counter=counter+1;
