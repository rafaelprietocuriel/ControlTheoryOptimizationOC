function varargout=constraint(ocObj,solObj,varargin)
%
...
if isempty(solObj)
    ...
    varargout{1}=feval(ocObj,'SymbolicConstraint',arcarg);
else
    ...
    if connectflag==0 || connectflag==1
        for ii=1:arcn
            ...
            c=feval(ocObj,'Constraint',indepvar(arcp-1),depvar(:,arcp),par,arcarg(ii),[]);
            if connectflag
                varargout{1}(:,arcp-1)=c;
            else
                varargout{ii}=c;
            end
        end
    elseif connectflag==2
        varargout{1}=feval(ocObj,'Constraint',indepvar(end),depvar(:,end),par,arcarg(end),[]);
    end
end
