...
if isempty(solObj)
    ...
    if connectflag==0 || connectflag==1
        for ii=1:arcn
            ...
            [lmmc lmsc]=feval(ocObj,'LagrangeMultiplier',indepvar(arcp-1),depvar(:,arcp),par,arcarg(ii),[]);
            if connectflag
                varargout{1}(:,arcp-1)=[lmmc;lmsc];
            else
                varargout{ii}=[lmmc;lmsc];
            end
        end
    elseif connectflag==2
            [lmmc lmsc]=feval(ocObj,'LagrangeMultiplier',indepvar(end),depvar(:,end),par,arcarg(end),[]);
            varargout{1}=[lmmc lmsc];
    end
end
