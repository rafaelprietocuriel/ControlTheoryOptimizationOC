...
elseif isdynprimitive(solObj) || isoctrajectory(solObj) || ishybridoctrajectory(solObj)
...
if isempty(solObj)
    ...
    for ii=1:arcn
        ...
        o=feval(ocObj,'ObjectiveFunction',indepvar(arcp-1),depvar(:,arcp),par,arcarg(ii),[]);
        if connectflag
            varargout{1}(arcp-1)=o;
        else
            varargout{ii}=o;
        end
    end
