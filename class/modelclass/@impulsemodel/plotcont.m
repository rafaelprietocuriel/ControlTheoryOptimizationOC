        ...
        if isstruct(solObj)
            for jj=contindex
                if strcmp(contfield,'ContinuationInformation')
                    switch contclass
                        ...
                        case {'extremalt4fimp'}
                            currentsolObj=hybridoctrajectory(solObj(jj).data.sol);
                    end
                else
                    switch contclass
                        case {'indifferencesolution','heteroclinic','indifferencedistribution'}
                        ...
                        case {'extremalt4fimp'}
                            currentsolObj=hybridoctrajectory(solObj(jj));
                    end
                end
                ...
                if length(htmp)==1
                    ...
                    for kk=1:length(htmp)
                        set(htmp(kk),'Tag',[tagname '_Nr:' num2str(ii) '_' num2str(jj) '_' num2str(kk)])
                    end
                end
                figure(gcf)
                %pause(0.1)
                if ~holdinter && (jj<contindex(end))
                    delete(htmp)
                else
                    h=[h;htmp];
                end
            ...
            if length(htmp)==1
                ...
                for jj=1:length(htmp)
                    set(htmp(jj),'Tag',[tagname '_Nr:' num2str(ii) '_' num2str(jj)])
                end
            end
            h=[h;htmp];
        end
