function savefigure(ocObj,varargin)
%
...
if isempty(savedir)
    savedir=fullocmatfile(userdatafolder(ocObj)); % Data directory
end
if isempty(figfilename)
    figfilename=filename(ocObj);
end
if isempty(figfilename)
...
savefigure(fighdl,fullfile(savedir,figfilename),imgtype,['-r' num2str(resolution)],varargin{6:end})