function [indiffpt,o,idx1,idx2]=findindifferencepoint(ocObj,player,sliceman1,sliceman2,varargin)
%
...
objectivevaluecoordinate=ocTrj1{1}.solverinfo.objectivevaluecoordinate(player);
contSol=contsolution(contRes{sliceman1});

sliceman1=occurve(slicemanifold(ocObj,sliceman1,contname));
objval1=zeros(1,length(contSol));
for ii=1:length(contSol)
    objval1(ii)=contSol(ii).y(objectivevaluecoordinate,end);
end
...
contSol=contsolution(contRes{sliceman2});
sliceman2=occurve(slicemanifold(ocObj,sliceman2,contname));

objval2=zeros(1,length(contSol));
for ii=1:length(contSol)
    objval2(ii)=contSol(ii).y(objectivevaluecoordinate,end);
end
