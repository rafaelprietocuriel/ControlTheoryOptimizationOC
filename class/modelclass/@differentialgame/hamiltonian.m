function varargout=hamiltonian(dgObj,solObj,varargin)
%
...
if isempty(solObj)
    ...
    varargout{1}=feval(dgObj,'SymbolicHamiltonian',arcarg,repflag);
else
    ...
    for ii=1:arcn
        ...
        c=feval(dgObj,'Hamiltonian',indepvar(arcp),depvar(:,arcp),par,arcarg(ii));
        if connectflag
            varargout{1}(:,arcp)=c;
        else
            varargout{ii}=c;
