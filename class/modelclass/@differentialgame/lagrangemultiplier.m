function varargout=lagrangemultiplier(dgObj,solObj,varargin)
%
...
if isempty(solObj)
    ...
    varargout{1}=feval(dgObj,'SymbolicConstraint',arcarg);
else
    ...
    for ii=1:arcn
        ...
        c=feval(dgObj,'LagrangeMultiplier',indepvar(arcp),depvar(:,arcp),par,arcarg(ii));
        if connectflag
            varargout{1}(:,arcp)=c;
        else
            varargout{ii}=c;
