function varargout=equilibriumequation(dgObj,solObj,varargin)
%
...
if ~isempty(solObj)
    ...
    for ii=1:arcn
        ...
        if strcmp(equationfile,'CanonicalSystem')
            dxdt=feval(dgObj,equationfile,0,depvar(:,arcp),parametervalue(dgObj),arcarg(ii));
        else
            dxdt=feval(dgObj,equationfile,depvar(:,arcp),parametervalue(dgObj),arcarg(ii));
        end
        varargout{ii}=dxdt;
    end
else
    ...
    if strcmp(symbolicequationfile,'SymbolicCanonicalSystem')
        varargout{1}=feval(dgObj,symbolicequationfile,arcarg,1);
    else
        varargout{1}=feval(dgObj,symbolicequationfile,arcarg);
    end
end
