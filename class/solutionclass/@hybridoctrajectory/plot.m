...
ocTrj=varargin{1};
varargin(1)=[];
if ~ishybridoctrajectory(ocTrj)
    ocmaterror('First or second argument is not an ''hybridoctrajectory''.')
end
...
%newplot
if ~isempty(axhdl)
    h=line(ocTrj,'xcoord',xcoord,'ycoord',ycoord,'xdata',xdata,'ydata',ydata,'Parent',axhdl,varargin{:});
else
    h=line(ocTrj,'xcoord',xcoord,'ycoord',ycoord,'xdata',xdata,'ydata',ydata,varargin{:});
end
if nargout==1
    varargout{1}=h;
end
