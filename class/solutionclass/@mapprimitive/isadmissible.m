function [b adminfo]=isadmissible(mapPrim,ocObj,varargin)
%
...
constraintval=admissible(ocObj,mapPrim);
zeroval=canonicalmap(ocObj,mapPrim);
imagval=imag(mapPrim);
if ~isempty(userfunc)
    userval=feval(ocObj,userfunc,[independentvar(mapPrim) independentvar(mapPrim)],[dependentvar(mapPrim) dependentvar(mapPrim)],parametervalue(ocObj),arcargument(mapPrim));
else
    userval=[];
end

if LegendreClebsch
    eigval=eig(d2hamiltoniandu2(ocObj,mapPrim));
    leclc=all(eigval<0);
end
