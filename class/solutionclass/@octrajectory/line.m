...
ocTrj=transform2multiarc(ocTrj);
if nargin<=2 
...
if isnumeric(varargin{1})
    if isnumeric(varargin{2})
        if nargin>=4 && isnumeric(varargin{3})
            [varargout{1:nargout}]=line(ocTrj,'xcoord',varargin{1},'ycoord',varargin{2},'zcoord',varargin{3},'xdata','dependentvar','ydata','dependentvar','zdata','dependentvar',varargin{4:nargin-1});
            return
        else
            [varargout{1:nargout}]=line(ocTrj,'xcoord',varargin{1},'ycoord',varargin{2},'xdata','dependentvar','ydata','dependentvar',varargin{3:nargin-1});
            return
        end
    else
        ocmaterror('Not enough input arguments.')
    ...
    function varargout=propvar2propval(propval)
        ...
        switch lower(propval)
            ...
            case {'objectivefunction'}
                [varargout{1:nargout}]=objectivefunction(ocObj,ocTrj,connectflag);
            ...
            case {'jacobian'}
                [varargout{1:nargout}]=jacobian(ocObj,ocTrj,[],[],connectflag);
            case {'adjointsystem'}
                [varargout{1:nargout}]=adjointsystem(ocObj,ocTrj);
            case {'spatialnorm'}
                [varargout{1:nargout}]=spatialnorm(ocObj,ocTrj);
            otherwise
