function writeimplicitcontroldynamicsjacobian(m,arcid,filename)
...
staten=statenum(m);
statecostatecoord=sprintf('1:%d',2*staten);
statecostatenum=sprintf('%d',2*staten);
...
for ii=arcid;
    [dUdX,dUdtsym,JUDX,JUDU]=generateimplicitcontroldynamics(m,ii);
    JU=[JUDX JUDU];
