...
if isdifferentialgame(ocObj)
    multiobjective=playernum(ocObj);
...
if ~isfield(sliceman1.userinfo,'objectivevalue')
    if ~isfinitesol1
        ...
        if ~multiobjective
            ...
            H=hamiltonian(ocObj,sliceman1,1);
            objval1=H./repmat(r,1,size(H,2));
        end
    else
        r=double(discountrate(ocObj));
...
if ~isempty(sliceman2)
    if ~isfield(sliceman2.userinfo,'objectivevalue')
        if ~isfinitesol2
            ...
            if ~multiobjective
                ...
                H=hamiltonian(ocObj,sliceman2,1);
                objval2=H./repmat(r,1,size(H,2));
            end
        else
            r=double(discountrate(ocObj));
    ...
    if ~multiobjective
        ...
        idx1=[];
        idx2=[];
        for ii=1:multiobjective
            [v0tmp,otmp,idx1tmp,idx2tmp]=intersections(v1,objval1(ii,:),v2,objval2(ii,:));
            if ~isempty(v0tmp)
                v0=[v0 v0tmp];
                o=[o otmp];
                idx1=[idx1 [idx1tmp;ii]];
                idx2=[idx2 [idx2tmp;ii]];
            end
        end
    end
else
...
for ii=1:length(v0)
    ...
    if PlotResult
        if ~multiobjective
            ...
            subplot(2,2,player)
            if ~isempty(sliceman2)
                h=plot(v1,objval1(player,:),v2,objval2(player,:),[v0(ii) v0(ii)],[min([objval1(player,:) objval2(player,:)]) max([objval1(player,:) objval2(player,:)])]);
            else
                h=plot(v1,objval1(player,:),[v0(ii) v0(ii)],[min(objval1(player,:)) max(objval1(player,:))]);
            end
            if ii==1
                hold on
            elseif ii==length(v0)
